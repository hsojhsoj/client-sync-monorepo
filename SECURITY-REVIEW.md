...
The single most worrying finding is **authorization granularity on the appointments REST controller and related PII reads**: `Appointments_Rest_Controller::check_permissions()` ([class-appointments-rest-controller.php:86](src/shared/includes/rest-api/class-appointments-rest-controller.php:86)) gates read access behind `current_user_can('edit_posts')`, which WordPress grants to Contributors, Authors, and Editors — not just practice managers. Every listed or fetched appointment then exposes `client.email`, `client.display_name`, the `notes` meta field, and (if the site is running HIPAA mode) encryption status. On any multi-author site this is a PII-leak vector; on a HIPAA-operational deployment it is a compliance issue.

Secondary concerns: the self-hosted Pro update channel has no package signature/hash verification (medium), and the seat-selection REST endpoints allow anonymous hold/release/refresh with only a rate-limit (medium, DoS-class).
