...
const { restUrl, get_nonce, save_nonce, ajax_url, l10n, editUrlBase, addNewApptUrlBase } = clisycManageSlotsData;
...
const ManageSlotsApp = () => {
    ...
    const showToast = (message, isError = false) => {
        const statusEl = document.getElementById('clisyc-manage-slots-save-status-text');
        if (statusEl) {
    ...
    const handleModalSave = () => {
        ...
        const currentIsBlock = document.getElementById('clisyc-dialog-slot-type')?.value === 'true';
        // --- END CRITICAL FIX ---
        ...
        if (currentIsBlock) {
            ...
            currentDimensions.forEach(dim => {
                ...
                if (selectedPostId) {
                    const post = dim.posts.find(p => p.value === selectedPostId);
                    if (post) { titleParts.push(post.label); }
                }
            });
            ...
            eventDataToAdd = {
                ...
                title: titleParts.length > 0 ? titleParts.join(' - ') : (l10n.available || 'Available'),
    ...
    const getEventSignature = (event) => {
        ...
        const dimString = sortedDimKeys.map(key => `${key}:${dims[key]}`).join(',');
        // FIX: Use ISO strings for dates, as startStr/endStr may not exist on manually created events
        const startStr = event.start ? event.start.toISOString() : event.startStr || '';
        const endStr = event.end ? event.end.toISOString() : event.endStr || '';
        return `${startStr}|${endStr}|${!!event.extendedProps.is_block}|${dimString}`;
    };
    ...
    const handleSaveChanges = useCallback(() => {
        setChangesMade(currentChanges => {
            ...
            const eventsToSave = calendar.getEvents().filter(e => !e.extendedProps.isBooked).map(e => ({
            ...
            window.jQuery.post(ajax_url, {
                ...
                if (response.success) {
                    showToast(response.data.message || l10n.saveSuccess || 'Slots saved.');
                    ...
                    showToast(response.data.message || l10n.saveError || 'Error saving.', true);
                }
            }).fail(() => showToast(l10n.saveError || 'Error saving.', true)).always(() => setIsSaving(false));
            return true;
        });
    }, []);
    ...
    const checkForChanges = useCallback(() => {
        ...
        const currentEvents = calendarInstanceRef.current.getEvents().filter(e => !e.extendedProps.isBooked);
        const currentSignatures = new Set(currentEvents.map(getEventSignature));
        let hasChanged = initialEventsState.current.size !== currentSignatures.size;
        if (!hasChanged) {
    ...
    useEffect(() => {
        ...
        const fetchDimensions = async () => {
            try {
                const enabledTypes = await apiFetch({ path: '/clisyc/v1/dimensions/types' });
                
                // CRITICAL FIX: Handle case where API doesn't return an array
                if (!Array.isArray(enabledTypes)) {
                    console.error("Dimensions API did not return an array:", enabledTypes);
                    setDimensions([]);
                    showToast('Warning: Could not load dimension types. Dimension fields may not be available.', true);
                    return;
                }
                
                const dimensionData = await Promise.all(
                    enabledTypes.map(async type => {
                        const posts = await apiFetch({ path: `/clisyc/v1/dimensions/posts?type=${type.slug}` });
                        return { ...type, posts: Array.isArray(posts) ? posts : [] };
                    })
                );
                ...
                setDimensions([]);
                showToast('Warning: Could not load dimensions. Please refresh the page.', true);
            }
        };
        fetchDimensions();
            ...
            calendar = new Calendar(calendarEl, {
                ...
                events: (fetchInfo, successCallback, failureCallback) => {
                    ...
                    apiFetch({ 
                        ...
                        .catch(error => { 
                            console.error('[FETCH] Error fetching slots:', error);
                ...
                select: (selectInfo) => {
                    ...
                    const initialDims = {};
                    const currentDimensions = currentStateRef.current.dimensions || [];
                    currentDimensions.forEach(dim => initialDims[dim.slug] = '');
                ...
                eventsSet: () => {
                    if (isInitialLoad.current) {
                        const initialEvents = calendarInstanceRef.current.getEvents().filter(e => !e.extendedProps.isBooked);
                        initialEventsState.current = new Set(initialEvents.map(getEventSignature));
                ...
                eventClick: (info) => {
                    ...
                    if (info.jsEvent.target.classList.contains('clisyc-delete-event')) { 
                        // Don't allow deleting booked events
                        if (props.isBooked) {
                            return;
                        }
                        if (window.confirm(l10n.confirmDelete || 'Are you sure you want to delete this slot?')) { 
                            info.event.remove(); 
                        }
                        return;
                    }
                    ...
                    if (props.isBooked && props.appointmentId) {
                        ...
                        if (editUrlBase) {
                            window.location.href = editUrlBase + props.appointmentId;
                        } else {
                ...
                eventContent: (arg) => {
                    ...
                    const deleteBtnHtml = !props.isBooked ? `<span class="clisyc-delete-event" title="${esc(l10n.confirmDelete || 'Delete Slot')}">×</span>` : '';
                    return {
                        html: `<div class="fc-event-main-frame"><div class="fc-event-time">${esc(arg.timeText)}</div><div class="fc-event-title-container"><div class="fc-event-title fc-sticky">${esc(arg.event.title)}</div></div>${deleteBtnHtml}</div>`
                    };
                },
                eventAllow: (dropInfo, draggedEvent) => !draggedEvent.extendedProps.isBooked,
            });
    ...
    useEffect(() => {
        const saveButton = calendarRef.current?.querySelector('.fc-saveAvailableSlotsButton-button');
        if (saveButton) {
    ...
    return (
        <>
            <div className="clisyc-admin-instructions">
                <p><strong>{l10n.whatIsThis || 'What is this?'}</strong> {l10n.whatIsThisDesc || 'This is the primary editor for making direct, manual changes to your calendar for a specific date range.'}</p>
                <p><strong>{l10n.whatCanIDo || 'What can I do here?'}</strong></p>
                <ul style={{ listStyle: 'disc', marginLeft: '20px' }}>
                    <li>{l10n.actionCreate || 'Create new slots by clicking and dragging on an empty area.'}</li>
                    <li>{l10n.actionModify || 'Modify existing slots by dragging them to a new time or resizing their duration.'}</li>
                    <li>{l10n.actionDelete || 'Delete a slot by clicking the (×) icon that appears on hover.'}</li>
                </ul>
                <p><em><strong>{l10n.importantNote || 'Important:'}</strong> {l10n.importantNoteDesc || 'Changes are not saved until you click the "Save Calendar Slots" button.'}</em></p>
                <div id="clisyc-manage-slots-save-status" style={{ textAlign: 'right', display: 'inline-block', float: 'right' }}>
                    <span id="clisyc-manage-slots-save-status-text" style={{ transition: 'opacity 0.3s', opacity: 0 }}></span>
                </div>
            </div>
            <div id="clisyc-manage-slots-calendar-container">
                {isLoading && ( <div className="clisyc-loading-overlay"><SkeletonCalendar /></div> )}
                <div ref={calendarRef}></div>
            </div>
            <div ref={modalRef} style={{ display: 'none' }}>
                <div className="clisyc-slot-modal">
                    <div className="clisyc-dialog-field">
                        <label htmlFor="clisyc-dialog-slot-type">{l10n.slotTypeLabel || 'Slot Type'}</label>
                        <select 
                            ...
                            onChange={e => {
                                ...
                                if (isBlocked) {
                                    const clearedDims = {};
                                    dimensions.forEach(dim => clearedDims[dim.slug] = '');
                    ...
                    {!modalIsBlock && dimensions.map(dim => (
                        <div key={dim.slug} className="clisyc-dialog-field">
                            <label htmlFor={`clisyc-dialog-dim-${dim.slug}`}>{dim.label}</label>
                            <select 
                                id={`clisyc-dialog-dim-${dim.slug}`} 
                                value={modalSelectedDims[dim.slug] || ''} 
                                onChange={e => {
                                    setModalSelectedDims(prev => ({ ...prev, [dim.slug]: e.target.value }));
                                }}
                            >
                                <option value="">-- {l10n.none || 'None'} --</option>
                                {dim.posts.map(post => (
