...
export default memo(({ data, isConnectable }) => {
    ...
    const childCountLabel = data.childCount > 0 ? ` (${data.childCount})` : '';

    return (
        <div className={nodeClasses.join(' ')} onDoubleClick={handleDoubleClick} title="Double-click to make this the primary dimension">
            <HandleWithTooltip
                type="target"
            ...
            <div className="clisyc-node-header">
                <span className={`clisyc-node-icon dashicons ${data.icon || 'dashicons-admin-generic'}`}></span>
                {data.isPrimary && <span className="clisyc-node-primary-star" title="Primary Dimension">★</span>}
                <a
                    href={data.admin_url}
                    target="_blank"
                ...
                <button
                    ...
                    title={data.isExpanded ? "Hide child items" : "Show child items"}
                >
                    <span className={`dashicons ${data.isExpanded ? 'dashicons-visibility' : 'dashicons-hidden'}`}></span>
                </button>
            </div>
