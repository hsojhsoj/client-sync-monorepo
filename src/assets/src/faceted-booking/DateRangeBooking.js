...
const DateRangeBooking = ({ 
    ...
    const allowedCheckinDays = primaryItem?.checkin_days || [0, 1, 2, 3, 4, 5, 6];
    const allowedCheckoutDays = primaryItem?.checkout_days || [0, 1, 2, 3, 4, 5, 6];
    ...
    useEffect(() => {
        ...
        const fetchBookedDates = async () => {
            ...
            try {
                ...
                const endDate = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString(); // 1 year ahead
    ...
    useEffect(() => {
        if (!pickerRef.current || isLoadingDates || !window.Litepicker) return;
            ...
            setup: (picker) => {
                picker.on('selected', (startDate, endDate) => {
                    if (startDate && endDate) {
                        ...
                        const error = validateDateRange(startStr, endStr, bookedSet);
                        if (error) {
    ...
    }, [bookedDates, isLoadingDates, minStay, maxStay, allowedCheckinDays, allowedCheckoutDays]);

    // Validate date range against booked dates
    const validateDateRange = useCallback((startStr, endStr, bookedSet) => {
        ...
        while (current < end) {
            ...
            if (bookedSet.has(dateStr)) {
                return l10n?.dateRangeConflict || 'Selected dates overlap with an existing booking.';
            }
        ...
        if (nights < minStay) {
            return `${l10n?.minStayRequired || 'Minimum stay is'} ${minStay} ${minStay === 1 ? 'night' : 'nights'}.`;
        }

        if (maxStay > 0 && nights > maxStay) {
            return `${l10n?.maxStayExceeded || 'Maximum stay is'} ${maxStay} ${maxStay === 1 ? 'night' : 'nights'}.`;
        }
    ...
    const handleBookNow = () => {
        if (!checkInDate || !checkOutDate) {
            setValidationError(l10n?.selectDates || 'Please select check-in and check-out dates.');
            return;
        }
    ...
    const primaryDimKey = Object.keys(selectedFilters).find(key => 
        ...
        <div className="clisyc-date-range-booking">
            <div className="clisyc-date-range-header">
                ...
                {minStay > 1 && (
                    <p className="clisyc-stay-requirement">
                        {l10n?.minimumStay || 'Minimum stay:'} {minStay} {minStay === 1 ? 'night' : 'nights'}
                    </p>
                )}
            </div>
            ...
            {isLoading ? (
                <div className="clisyc-loading-indicator">
                    <span className="spinner is-active" style={{ float: 'none', margin: '0 10px 0 0' }}></span>
                    {l10n?.loadingAvailability || 'Loading availability...'}
                </div>
            ) : (
                <>
                    <div className="clisyc-date-picker-container">
                        <label>{l10n?.selectDates || 'Select your dates:'}</label>
                        <input
                            ref={pickerRef}
                            type="text"
                            className="clisyc-date-range-input"
                            placeholder={l10n?.checkInCheckOut || 'Check-in → Check-out'}
                            readOnly
                        />
                    </div>
                    ...
                    {checkInDate && checkOutDate && !validationError && (
                        <div className="clisyc-selected-dates-summary">
                            <div className="clisyc-date-summary-row">
                                <span className="clisyc-date-label">{l10n?.checkIn || 'Check-in:'}</span>
                                <span className="clisyc-date-value">{formatDateDisplay(checkInDate)}</span>
                            </div>
                            <div className="clisyc-date-summary-row">
                                <span className="clisyc-date-label">{l10n?.checkOut || 'Check-out:'}</span>
                                <span className="clisyc-date-value">{formatDateDisplay(checkOutDate)}</span>
                            </div>
                            <div className="clisyc-date-summary-row clisyc-nights-total">
                                <span className="clisyc-date-label">{l10n?.totalNights || 'Total:'}</span>
                    ...
                    <button
                        ...
                        disabled={!checkInDate || !checkOutDate || !!validationError}
                    >
            ...
            {/* Confirmation Modal */}
            {isConfirmationOpen && (
                <div className="clisyc-compact-modal-overlay" onClick={() => setIsConfirmationOpen(false)}>
                    <div className="clisyc-compact-modal-content" onClick={e => e.stopPropagation()}>
                        <form method="post" id="clisyc_appointment_form" action="" noValidate>
                            ...
                            <input type="hidden" name="start_date" value={checkInDate} />
                            <input type="hidden" name="end_date" value={checkOutDate} />
                            <input type="hidden" name={primaryDimKey} value={itemId} />
                            <input type="hidden" name="clisyc_appointment_nonce" value={formNonce} />
                            
                            {/* Include other selected filters */}
                            {Object.entries(selectedFilters).map(([key, values]) => {
                                if (key === primaryDimKey) return null;
                                return values.map((value, idx) => (
                                    <input type="hidden" name={key} value={value} key={`${key}-${idx}`} />
                                ));
                            })}
                            ...
                            <div className="clisyc-confirmation-details">
                                ...
                                <div className="clisyc-confirmation-dates">
                                    <p>
                                        <strong>{l10n?.checkIn || 'Check-in:'}</strong> {formatDateDisplay(checkInDate)}
                                    </p>
                                    <p>
                                        <strong>{l10n?.checkOut || 'Check-out:'}</strong> {formatDateDisplay(checkOutDate)}
                                    </p>
                                    <p className="clisyc-nights-highlight">
                                        <strong>{calculateNights()}</strong> {calculateNights() === 1 ? 'night' : 'nights'}
                                    </p>
                                </div>
                            </div>
                            ...
                            >
                                {paymentRequired 
                                    ...
                                    : (l10n?.confirmReservation || 'Confirm Reservation')
                                }
                            </button>
                            <button 
                                type="button" 
