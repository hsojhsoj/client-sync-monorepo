...
const App = () => {
    ...
    const timezone = calendarOptions?.timeZone || Intl.DateTimeFormat().resolvedOptions().timeZone;
    ...
    const primaryDimKey = useMemo(() => {
        ...
        if (registry?.dimensions) {
            for (const [slug, settings] of Object.entries(registry.dimensions)) {
                if (settings.primary) return slug;
            }
        }
    ...
    const selectedPrimaryItem = useMemo(() => {
        if (!primaryDimKey || !selectedFilters[primaryDimKey]?.length) return null;
        
        const selectedId = selectedFilters[primaryDimKey][0];
        const options = allFilterOptions[primaryDimKey] || [];
        return options.find(opt => String(opt.value) === String(selectedId)) || null;
    ...
    const bookingMode = useMemo(() => {
        ...
        if (primaryOptions.length > 0) {
            const allDateRange = primaryOptions.every(opt => opt.booking_mode === 'date_range');
            if (allDateRange) return 'date_range';
        }
        return 'slot';
    ...
    const hasActiveFilters = useMemo(() => {
        return Object.values(selectedFilters).some(val => Array.isArray(val) && val.length > 0);
    }, [selectedFilters]);
    ...
    const slotCountsByFilter = useMemo(() => {
        ...
        filterOrder.forEach(dimKey => {
            ...
            options.forEach(opt => {
                const matchingSlots = allSlots.filter(slot => {
                    if (slot.extendedProps?.is_block) return false;
                    if (!showBookedSlots && slot.extendedProps?.isBooked) return false;
                    if (!showBookedSlots && slot.extendedProps?.spotsLeft === 0) return false;
                    
                    const slotDimensions = slot.extendedProps?.dimensions || {};
                    const dimValue = slotDimensions[dimKey];
                    
                    if (String(dimValue) !== String(opt.value)) return false;
                    
                    for (const [filterKey, filterValues] of Object.entries(selectedFilters)) {
                        if (filterKey === dimKey) continue;
        ...
        return counts;
    }, [allSlots, allFilterOptions, filterOrder, selectedFilters, showBookedSlots]);

    const fetchData = useCallback(async (currentSelections) => {
        ...
        try {
            ...
            Object.entries(currentSelections).forEach(([key, values]) => {
            ...
            if (selectedPrimaryId && optionsResponse[primaryKey]) {
                ...
                if (primaryOptions.length > 0) {
                    const allDateRange = primaryOptions.every(opt => opt.booking_mode === 'date_range');
                    if (allDateRange) itemBookingMode = 'date_range';
                }
            }

            if (itemBookingMode === 'slot') {
                const isFirstLoad = Object.keys(currentSelections).length === 0;

                const slotsResponse = await apiFetch({
                    path: `/clisyc/v1/slots?${queryParams.toString()}&start=${new Date().toISOString()}&end=${new Date(new Date().setDate(new Date().getDate() + 90)).toISOString()}`,
                    signal: controller.signal
                });
    ...
    const validateGuestForm = useCallback(() => {
        ...
        if (!guestForm.email.trim()) {
            ...
            errors.push(l10n?.emailInvalid || 'Please enter a valid email address.');
        }
        if (!guestForm.password) {
            errors.push(l10n?.passwordRequired || 'Password is required.');
        } else if (guestForm.password.length < 8) {
            errors.push(l10n?.passwordTooShort || 'Password must be at least 8 characters.');
        }
    ...
    const timezoneDisplay = useMemo(() => {
        ...
        const tzAbbr = currentTime.toLocaleTimeString('en-US', { 
            timeZoneName: 'short',
            timeZone: tz
        }).split(' ').pop();
            ...
            if (!selectedPrimaryItem) {
                return (
                    <div className="clisyc-booking-placeholder">
                        <p>{l10n?.selectPropertyForCalendar || 'Please select a property to view the availability calendar.'}</p>
                    </div>
                );
            }
            return (
    ...
    return (
        <>
            ...
            {isConfirmationModalOpen && selectedSlot && bookingMode === 'slot' && (
                <div className="clisyc-compact-modal-overlay" onClick={() => setIsConfirmationModalOpen(false)}>
                    <div className="clisyc-compact-modal-content clisyc-booking-modal" onClick={e => e.stopPropagation()}>
                        <form 
                            method="post" 
                        ...
                        >
                            ...
                            <input type="hidden" name="time_slot" value={formatTimeSlotForBackend(selectedSlot.start)} />
                            <input type="hidden" name="appointment_duration_minutes" value={selectedSlot.durationMinutes || 60} />
                            <input type="hidden" name="appointment_date" value={new Date(selectedSlot.start).toISOString().split('T')[0]} />                           
                            <input type="hidden" name="clisyc_appointment_nonce" value={formNonce} />
                            
                            {/* Dimension fields */}
                            {selectedSlot.dimensions && Object.entries(selectedSlot.dimensions).map(([key, value]) => (
                            ...
                            <div className="clisyc-confirmation-details">
                                <p><strong>{l10n?.youAreBooking || 'You are booking:'}</strong></p>
                                <p>{selectedSlot.fullDescription}</p>
                                <hr/>
                                <p>
                                    <strong>{l10n?.time || 'Time'}:</strong>{' '}
                                    {new Date(selectedSlot.start).toLocaleDateString([], {
                                        weekday: 'long', 
                                        month: 'long', 
                                        day: 'numeric'
                                    })} at {new Date(selectedSlot.start).toLocaleTimeString([], { 
                                        hour: 'numeric', 
                                        minute: '2-digit' 
                                    })}
                                </p>
                            ...
                            {isGuest && (
                                <div className="clisyc-guest-registration-section">
                                    <h4>{l10n?.createAccount || 'Create Your Account'}</h4>
                                    <p className="clisyc-guest-info">
                                        {l10n?.guestAccountInfo || 'Enter your details to create an account and complete your booking.'}
                                    </p>
                                    
                                    {formErrors.length > 0 && (
                                        <div className="clisyc-form-errors">
                                            {formErrors.map((error, index) => (
                                                <p key={index} className="clisyc-error-message">{error}</p>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    <div className="clisyc-form-row">
                                        <div className="clisyc-form-field">
                                            <label htmlFor="clisyc_reg_first_name">
                                                {l10n?.firstName || 'First Name'} <span className="required">*</span>
                                            </label>
                                            <input
                                                type="text"
                                        ...
                                        <div className="clisyc-form-field">
                                            <label htmlFor="clisyc_reg_last_name">
                                                {l10n?.lastName || 'Last Name'} <span className="required">*</span>
                                            </label>
                                            <input
                                                type="text"
                                    ...
                                    <div className="clisyc-form-field">
                                        <label htmlFor="clisyc_reg_email">
                                            {l10n?.email || 'Email Address'} <span className="required">*</span>
                                        </label>
                                        <input
                                            type="email"
                                            id="clisyc_reg_email"
                                    ...
                                    <div className="clisyc-form-field">
                                        <label htmlFor="clisyc_reg_password">
                                            {l10n?.password || 'Password'} <span className="required">*</span>
                                        </label>
                                        <input
                                            type="password"
                                        ...
                                        <span className="clisyc-field-hint">
                                            {l10n?.passwordHint || 'Minimum 8 characters'}
                                        </span>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="clisyc-modal-actions">
                                ...
                                >
                                    {paymentRequired 
                                        ? (l10n?.proceedToCheckout || 'Proceed to Checkout') 
                                        : (l10n?.scheduleAppointment || 'Schedule Appointment')
                                    }
                                </button>
                                <button 
                                    type="button" 
