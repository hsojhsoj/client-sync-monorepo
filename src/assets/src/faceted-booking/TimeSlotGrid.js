...
const TimeSlotGrid = React.memo(({
    ...
    const { availableSlots, bookedSlots } = useMemo(() => {
        ...
        slots.forEach(slot => {
            ...
            if (slot.extendedProps?.is_block) return;
            
            // Check if booked
            const isBooked = slot.extendedProps?.isBooked || slot.extendedProps?.spotsLeft === 0;
    ...
    const displaySlots = useMemo(() => {
        if (showBookedSlots) {
            ...
            return [...availableSlots, ...bookedSlots].sort((a, b) => 
    ...
    const getFullSlotDescription = useCallback((slot) => {
    ...
    const getSlotColors = useCallback((slot) => {
        const details = slot.extendedProps.serviceDetails || {};
        const colors = [];
    ...
    const isSlotBooked = useCallback((slot) => {
        return slot.extendedProps?.isBooked || slot.extendedProps?.spotsLeft === 0;
    }, []);

    // Group slots by date (memoized to avoid recalculation on every render)
    const slotsByDate = useMemo(() => displaySlots.reduce((acc, slot) => {
        ...
        <div className="clisyc-timeslot-header">
            <div className="clisyc-header-left">
                <span className="clisyc-slots-count">
                    <strong>{availableSlots.length}</strong> {availableSlots.length === 1 ? 'time' : 'times'} available
                </span>
            </div>
            <div className="clisyc-header-right">
                {/* Timezone display */}
                {timezoneDisplay && (
                    <div className="clisyc-timezone-display" title={timezoneDisplay.fullTimezone}>
                        <span className="dashicons dashicons-clock"></span>
                        <span className="clisyc-current-time">{timezoneDisplay.time}</span>
                        <span className="clisyc-timezone-abbr">{timezoneDisplay.timezone}</span>
                    </div>
                )}
                ...
                {bookedSlots.length > 0 && onToggleBookedSlots && (
                    <label className="clisyc-booked-toggle">
                        ...
                        <span className="clisyc-toggle-label">
                            {l10n?.showBooked || 'Show booked'} ({bookedSlots.length})
                        </span>
                    </label>
                )}
            </div>
        ...
        return (
            <div className="clisyc-timeslot-grid-wrapper">
                ...
                {hasActiveFilters ? (
                    <EmptyState
                        icon="search"
                        title={l10n?.noMatchingSlots || 'No available times match your selected filters.'}
                        message={l10n?.tryDifferentFilters || 'Try selecting different options or reset filters to see all availability.'}
                    />
                ) : (
                    <EmptyState
                        icon="calendar"
                        title={l10n?.noSlotsAvailable || 'No appointment times are currently available.'}
                        message={l10n?.checkBackLater || 'Please check back later or contact us for assistance.'}
                    />
                )}
            </div>
        );
                ...
                {Object.entries(slotsByDate).map(([date, dateSlots]) => (
                    <div key={date} className="clisyc-date-group">
                        ...
                        <div className="clisyc-slots-container">
                            {dateSlots.map(slot => {
                                const slotColors = getSlotColors(slot);
                                ...
                                return (
                                    <button 
                                        key={slot.id} 
                                        type="button" 
                                        className={`clisyc-slot-button ${booked ? 'clisyc-slot-booked' : ''}`}
                                        onClick={() => !booked && onSlotSelect({ 
                                            ...slot.extendedProps, 
                                            start: slot.start,
                                            fullDescription: getFullSlotDescription(slot)
                                        })}
                                        disabled={booked}
                                        title={booked ? (l10n?.slotBooked || 'This time is already booked') : ''}
                                    >
                                        ...
                                        <span className="clisyc-slot-button-details">
                                            {booked && <span className="clisyc-booked-badge">{l10n?.booked || 'Booked'}</span>}
                                            {getFullSlotDescription(slot)}
                                        </span>
                                    </button>
                                );
                            })}
