...
const FacetedFilterPanel = React.memo(
	( {
		...
		const { availabilityDimensions, filterOrder, l10n } =
		...
		const totalSlots = useMemo(
			() =>
				Object.values( slotCounts ).reduce( ( total, dimCounts ) => {
					const maxInDim = Math.max(
						...Object.values( dimCounts ),
						0
					);
				...
				<div className="clisyc-filter-group-wrapper">
					...
					{ totalSlots > 0 && (
						<div className="clisyc-filter-total-count">
							<span className="clisyc-total-number">
								{ totalSlots }
							</span>
							<span className="clisyc-total-label">
								{ totalSlots === 1
									? l10n?.appointmentAvailable ||
									  'appointment available'
									: l10n?.appointmentsAvailable ||
									...
									{ currentOptions.map( ( opt ) => {
										...
										return (
											...
											>
												...
												{ /* Slot count badge */ }
												<span
													className={ `clisyc-filter-count-badge ${
														! hasSlots
															? 'clisyc-count-zero'
															: ''
													}` }
