...
const CompactTimeSlotGrid = ( {
	...
	const gridStartHour = parseInt(
		calendarOptions.slotMinTime?.split( ':' )[ 0 ] || '8',
		10
	);
	const gridEndHour = parseInt(
		calendarOptions.slotMaxTime?.split( ':' )[ 0 ] || '19',
		10
	);
	...
	const timeLabels = Array.from( { length: totalHours }, ( _, i ) => {
		...
		return new Date( 1970, 0, 1, hour ).toLocaleTimeString( [], {
			hour: 'numeric',
		} );
	} );
	...
	useEffect( () => {
		if ( ! onSlotCountsUpdate ) {
			return;
		}
		if ( ! hasFetched ) {
			return;
		} // Don't report counts until first fetch completes
		...
		const availableSlots = allSlots.filter( ( s ) => {
			...
			if ( s.spotsLeft === 0 || s.spots_left === 0 ) {
				return false;
			}
		...
		filterOrder.forEach( ( filterKey ) => {
			...
			availableSlots.forEach( ( slot ) => {
				...
				if ( dimensionId ) {
					byFilter[ filterKey ][ dimensionId ] =
						( byFilter[ filterKey ][ dimensionId ] || 0 ) + 1;
				}
			} );
		} );
				...
				if ( response && response.events ) {
					response.events.forEach( ( event ) => {
						...
						const slotData = {
							...
							timeText: startLocal.toLocaleTimeString( [], {
								hour: 'numeric',
								minute: '2-digit',
							} ),
							title: event.title,
						...
						if ( event.extendedProps?.isBooked ) {
							if ( ! organizedBookedSlots[ dateKey ] ) {
								organizedBookedSlots[ dateKey ] = [];
							}
							organizedBookedSlots[ dateKey ].push( slotData );
						} else {
							if ( ! organizedSlots[ dateKey ] ) {
	...
	const handleSlotGroupClick = ( slots ) => {
		const validSlots = slots.filter( ( slot ) => {
			...
			if ( slot.spotsLeft === 0 || slot.spots_left === 0 ) {
				return false;
			}
	...
	const getFullSlotDescription = ( slot ) => {
		...
		if ( descriptionParts.length === 0 ) {
			...
			if ( anyNames.length > 0 ) {
				return anyNames.join( ' with ' );
			}
			if ( slot.timeText ) {
				return `Available at ${ slot.timeText }`;
			}
	...
	const handleModalSlotSelect = ( slot ) => {
		...
		const displayTime = `${ new Date( slot.start ).toLocaleDateString( [], {
			weekday: 'long',
			month: 'long',
			day: 'numeric',
		} ) } at ${ slot.timeText }`;
		...
		for ( let i = 1; i < sorted.length; i++ ) {
			...
			if ( gap <= 60000 ) {
				current = {
					...
					slotIdentifier:
						current.slotIdentifier + '+' + next.slotIdentifier,
				};
			} else {
				merged.push( current );
				current = next;
	...
	return (
		<div className="clisyc-compact-timeslot-grid-container">
			{ isLoading ? (
				...
				<div
					className="clisyc-compact-grid-loading"
					role="status"
				...
				<div
					className={ `clisyc-compact-grid-layout ${
						isMobileView ? 'mobile-view' : ''
					}` }
				>
					<div
						className="clisyc-compact-time-gutter"
						ref={ timeGutterRef }
					...
					>
						{ timeLabels.map( ( label ) => (
							<div
								key={ label }
								className="clisyc-compact-time-label"
							>
								{ label }
							</div>
						) ) }
					...
					{ dates.map( ( date, index ) => {
						...
						return (
							<div
								key={ dateKey }
								className="clisyc-compact-grid-day-column"
							>
								<div
									ref={
										index === 0 ? firstDayHeaderRef : null
									}
									className={ `clisyc-compact-grid-day-header ${
										isToday ? 'clisyc-compact-today' : ''
									}` }
								>
									<div className="clisyc-compact-day-name">
										{ date
											.toLocaleDateString( [], {
												weekday: 'short',
											} )
									...
									<div className="clisyc-compact-day-date">
								...
								<div
									ref={
										index === 0
											? firstDaySlotContainerRef
											: null
									}
									className="clisyc-compact-grid-day-slots"
									style={ {
										height: `${ totalHours * 60 }px`,
									} }
								>
									{ isToday && indicatorTop !== null && (
										<div
											className="clisyc-compact-time-indicator"
											style={ {
												top: `${ indicatorTop }px`,
											} }
										></div>
									...
									{ ( () => {
										const rawBlockedSlots = daySlots.filter(
											( s ) => s.is_block
										);
										const blockedSlots = showBlocked
											? mergeConsecutiveBlocks(
													rawBlockedSlots
											  )
											: [];
										const bookableSlots = daySlots.filter(
											( s ) => ! s.is_block
										);

										const slotsByHour =
											bookableSlots.reduce(
												( acc, slot ) => {
													...
													( [ hourKey, group ] ) => {
														...
														const isGroupSelected =
															...
															availableSlots.some(
																( s ) =>
																	s.slotIdentifier ===
																	selectedSlot.identifier
															);

														const uniqueColors =
															Array.from(
																new Set(
																	availableSlots.flatMap(
																		(
																			slot
																		) => {
																			const details =
														...
														return (
															<button
																key={ hourKey }
																type="button"
																className={ `clisyc-compact-slot-item clisyc-compact-slot-available ${
																	isGroupSelected
																		? 'clisyc-compact-slot-selected'
																		: ''
																}` }
																style={ getSlotStyle(
																	group.start,
			...
			{ isModalOpen && (
				...
				>
					...
					>
						...
						<div
							className="clisyc-compact-modal-slots-list"
							role="listbox"
							aria-labelledby="clisyc-compact-modal-title"
						>
							{ modalData.slots.map( ( slot ) => {
								...
								return (
									<button
										...
										className={ `clisyc-compact-time-slot-button ${
											isSelected
												? 'clisyc-compact-slot-selected'
												: ''
										}` }
										onClick={ () =>
											handleModalSlotSelect( slot )
										}
									>
										<div className="clisyc-compact-slot-dots">
											{ Array.from(
										...
										<div className="clisyc-compact-filter-label-content">
											<span className="clisyc-compact-filter-label-text">
												{ getFullSlotDescription(
													slot
												) }
											</span>
											{ slot.durationMinutes > 0 && (
												<span className="clisyc-compact-filter-duration">
