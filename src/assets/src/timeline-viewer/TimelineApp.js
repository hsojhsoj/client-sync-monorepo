 ...
 */
const ZOOM_LEVELS = [
    { label: '6h',  slotDuration: '06:00:00', slotMinWidth: 80,  labelInterval: '12:00:00' },
    { label: '3h',  slotDuration: '03:00:00', slotMinWidth: 60,  labelInterval: '06:00:00' },
    { label: '1h',  slotDuration: '01:00:00', slotMinWidth: 50,  labelInterval: '02:00:00' },
    { label: '30m', slotDuration: '00:30:00', slotMinWidth: 45,  labelInterval: '01:00:00' },
    { label: '15m', slotDuration: '00:15:00', slotMinWidth: 40,  labelInterval: '00:30:00' },
    { label: '10m', slotDuration: '00:10:00', slotMinWidth: 35,  labelInterval: '00:30:00' },
    { label: '5m',  slotDuration: '00:05:00', slotMinWidth: 30,  labelInterval: '00:15:00' },
];
...
const TimelineApp = ({ containerId }) => {
    ...
    const configKey = `clisycTimelineData_${containerId.replace(/-/g, '_')}`;
    ...
    const [containerHeight, setContainerHeight] = useState(parseInt(height) || 700);
    ...
    useEffect(() => {
        try {
            const savedHeight = localStorage.getItem('clisyc_timeline_height');
            if (savedHeight) {
                setContainerHeight(Math.max(300, Math.min(2000, parseInt(savedHeight))));
            }
    ...
    useEffect(() => {
        try {
            localStorage.setItem('clisyc_timeline_height', containerHeight.toString());
    ...
    useEffect(() => {
        localStorage.setItem('clisyc_timeline_compact', isCompactView.toString());
    }, [isCompactView]);
    ...
    const fcResources = useMemo(() => {
        return resources.map(resource => ({
            ...
            extendedProps: {
                ...
                cptTypeLabel: resource.type ? resource.type.replace('clisyc_', '').replace(/_/g, ' ') : '',
    ...
    const handleResizeStart = useCallback((e) => {
        ...
        const handleMove = (moveEvent) => {
            ...
            const newHeight = Math.max(300, Math.min(2000, startHeight + delta));
     ...
     */
    const renderResourceLabel = useCallback((arg) => {
        ...
        return (
            <div 
                ...
                onClick={handleClick}
                title={l10n.clickToEdit || 'Click to edit resource settings'}
    ...
    const fetchEventsSource = useCallback(async (fetchInfo, successCallback, failureCallback) => {
        try {
            ...
            if (response.dimension_types && response.dimension_types.length > 0) {
            ...
            const events = appointments.map(appt => {
                ...
                if (appt.is_blocked) {
                    ...
                    backgroundColor = colorSettings.available_bg || '#10b981';
    ...
    const handleEventClick = useCallback((info) => {
        ...
        if (props.is_booked && props.edit_link) {
            ...
            window.open(`${adminUrl}post.php?post=${props.resource_id}&action=edit`, '_blank');
        }
    }, [adminUrl]);
     ...
     */
    const getDimensionColor = useCallback((dimId) => {
    ...
    const renderEventContent = useCallback((eventInfo) => {
        ...
        if (Object.keys(dimensions).length > 0) {
            tooltipLines.push('---');
            Object.entries(dimensions).forEach(([dimType, dimName]) => {
        ...
        if (props.capacity && props.capacity > 1) {
            tooltipLines.push(`Capacity: ${props.booked_count || 0}/${props.capacity}`);
        }
        ...
        if (isCompactView) {
            ...
            return (
                <div className="clisyc-timeline-event-content" title={tooltipText}>
        ...
        return (
            <div className="clisyc-timeline-event-content clisyc-timeline-event-expanded" title={tooltipText}>
                <div className="clisyc-timeline-event-header">
                    <span className="fc-event-title">{event.title}</span>
                    {props.time_label && (
                        <span className="clisyc-timeline-event-time">{props.time_label}</span>
                    )}
                </div>
                {hasDimensions && (
                    <div className="clisyc-timeline-event-dimensions">
                        {Object.entries(dimensions).map(([dimType, dimName]) => {
                            ...
                            const dimResource = resources.find(r => 
                                r.type === dimType && r.name === dimName
                            );
                            const dimColor = dimResource?.color || getDimensionColor(dimResource?.id) || '#6b7280';
                            
                            return (
                                <span key={dimType} className="clisyc-timeline-dimension-item">
                                    <span 
                                        className="clisyc-timeline-dimension-dot"
                                        style={{ backgroundColor: dimColor }}
                                    />
                                    <span className="clisyc-timeline-dimension-name">{dimName}</span>
                                </span>
                            );
                        })}
                    </div>
    ...
    return (
        <div 
            className={`clisyc-timeline-wrapper ${isResizing ? 'clisyc-timeline-resizing' : ''} ${isCompactView ? 'clisyc-timeline-compact' : 'clisyc-timeline-expanded'}`}
            ref={wrapperRef}
        ...
        >
            ...
            <div className="clisyc-timeline-zoom-toolbar">
                <div className="clisyc-timeline-zoom-controls">
                    ...
                    <span className="clisyc-timeline-zoom-level" title={l10n.currentZoom || 'Current zoom level'}>
                        {currentZoom.label}
                    </span>
                    <button 
                        type="button"
                    ...
                    <button 
                        ...
                        title={l10n.zoomReset || 'Reset zoom to default (1 hour)'}
                    >
                ...
                <div className="clisyc-timeline-view-toggle">
                    <button 
                        type="button"
                        className={`clisyc-timeline-compact-btn ${isCompactView ? 'is-active' : ''}`}
                        onClick={handleToggleCompact}
                        title={isCompactView ? (l10n.expandView || 'Expand rows to show full text') : (l10n.compactView || 'Compact rows (show details on hover)')}
                    >
                        <span className={`dashicons ${isCompactView ? 'dashicons-editor-expand' : 'dashicons-editor-contract'}`}></span>
                        <span className="clisyc-timeline-toggle-label">
                            {isCompactView ? (l10n.compact || 'Compact') : (l10n.expanded || 'Expanded')}
                        </span>
                    </button>
                </div>
            ...
            <div className="clisyc-timeline-calendar-container" style={{ height: calendarHeight, opacity: isInitialLoading ? 0 : 1, transition: 'opacity 0.3s' }}>
                <FullCalendar
                    ref={calendarRef}
                    plugins={[ resourceTimelinePlugin, interactionPlugin ]}
                    schedulerLicenseKey="GPL-My-Project-Is-Open-Source"
                    initialView={initialViewMap[viewMode] || 'resourceTimelineWeek'}
                    timeZone="local"
                    height="100%"
                    ...
                    resourceAreaHeaderContent={l10n.selectResource || "Resources"}
