 ...
 */
export function SkeletonPulse( { width = '100%', height = '16px', radius = '4px', style = {} } ) {
	return (
		<div
...
export function SkeletonCard( { lines = 3 } ) {
	return (
		<div className="clisyc-skeleton-card" aria-label="Loading" role="status">
			<SkeletonPulse height="20px" width="60%" style={ { marginBottom: '16px' } } />
...
export function SkeletonTable( { rows = 5, columns = 4 } ) {
	return (
		<div className="clisyc-skeleton-table" aria-label="Loading" role="status">
			...
			{ Array.from( { length: rows }, ( _, r ) => (
				<div key={ r } className="clisyc-skeleton-table-row">
					{ Array.from( { length: columns }, ( _, c ) => (
						<SkeletonPulse key={ c } height="14px" width={ c === 0 ? '70%' : '50%' } />
					) ) }
				</div>
			) ) }
		</div>
...
export function SkeletonCalendar() {
	return (
		<div className="clisyc-skeleton-calendar" aria-label="Loading calendar" role="status">
			{/* Toolbar */}
			...
			{ Array.from( { length: 5 }, ( _, r ) => (
				<div key={ r } className="clisyc-skeleton-calendar-row">
					{ Array.from( { length: 7 }, ( _, c ) => (
						<div key={ c } className="clisyc-skeleton-calendar-cell">
							<SkeletonPulse width="24px" height="14px" style={ { marginBottom: '6px' } } />
							{ r < 3 && <SkeletonPulse width="90%" height="10px" /> }
						</div>
					) ) }
				</div>
			) ) }
...
export function SkeletonTimeline( { resources = 4 } ) {
	return (
		<div className="clisyc-skeleton-timeline" aria-label="Loading timeline" role="status">
			{/* Header row */}
...
export function SkeletonFilters( { filters = 3 } ) {
	return (
		<div className="clisyc-skeleton-filters" aria-label="Loading filters" role="status">
			{ Array.from( { length: filters }, ( _, i ) => (
				<div key={ i } className="clisyc-skeleton-filter-group">
					<SkeletonPulse width="90px" height="14px" style={ { marginBottom: '8px' } } />
