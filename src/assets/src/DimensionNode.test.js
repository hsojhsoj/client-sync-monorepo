...
jest.mock( './HandleWithTooltip', () => () => (
	<div data-testid="mock-handle" />
) );
...
describe( 'DimensionNode', () => {
	...
	it( 'renders the basic node information including label, child count, and slug', () => {
		...
		expect(
			document.querySelector( '.dashicons-clipboard' )
		).toBeInTheDocument();
	} );
	...
	it( 'displays the primary star icon only when isPrimary is true', () => {
		...
		expect(
			screen.queryByTitle( 'Primary Dimension' )
		).not.toBeInTheDocument();

		// Now, re-render the SAME component with the isPrimary prop set to true
		const primaryProps = {
			...defaultProps,
			data: { ...defaultProps.data, isPrimary: true },
		};
	...
	it( 'calls onSetPrimary with its slug on double click when it is NOT primary', async () => {
		...
		expect( defaultProps.data.onSetPrimary ).toHaveBeenCalledWith(
			'clisyc_service'
		);
	} );
	...
	it( 'calls onExpandToggle when the visibility icon is clicked', async () => {
		...
		expect( defaultProps.data.onExpandToggle ).toHaveBeenCalledWith(
			'clisyc_service',
			true
		);
	} );
	...
	it( 'shows the organize button and calls onOrganizeChildren only when expanded', async () => {
		...
		const { rerender } = render( <DimensionNode { ...defaultProps } /> );
		expect(
			screen.queryByTitle( 'Organize child nodes' )
		).not.toBeInTheDocument();

		// Re-render as expanded
		const expandedProps = {
			...defaultProps,
			data: { ...defaultProps.data, isExpanded: true },
		};
		...
		expect( expandedProps.data.onOrganizeChildren ).toHaveBeenCalledTimes(
			1
		);
		expect( expandedProps.data.onOrganizeChildren ).toHaveBeenCalledWith(
			'clisyc_service'
		);
	} );
} );
