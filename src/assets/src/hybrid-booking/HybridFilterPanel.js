            ...
            <div className="clisyc-filter-group-wrapper">
                ...
                <div className="clisyc-total-slots-container">
                    <div className="clisyc-total-slots-badge">
                        <span className="clisyc-total-slots-number">{slotCounts.total || 0}</span>
                        <span className="clisyc-total-slots-label">{l10n?.slotsInView || 'slots in current view'}</span>
                    </div>
                </div>
                ...
                <div className="clisyc-reset-all-container">
                    <button 
                        type="button" 
                        className={`clisyc-reset-all-filters-btn ${hasSelection ? 'clisyc-has-selections' : ''}`}
                        onClick={onReset}
                    ...
                    >
                        ...
                        <span className="clisyc-reset-text">
                            ...
                            {activeFilterCount > 0 && (
                                <span className="clisyc-active-filter-count">({activeFilterCount})</span>
                            )}
                        </span>
                    </button>
                </div>
                ...
                {filterOrder.map(key => {
                    ...
                    const selectedCount = (selections[key] || []).length;

                    return (
                        <div key={key} className="clisyc-filter-group" style={{ display: hasAvailableOptions ? 'block' : 'none' }}>
                            <h4 className="clisyc-filter-group-label">
                                <span className={`dashicons ${dim.icon || 'dashicons-admin-generic'}`}></span>
                                <span className="clisyc-filter-group-title">{dim.label}</span>
                                {selectedCount > 0 && (
                                    <span className="clisyc-filter-group-selected-count">{selectedCount}</span>
                                )}
                            </h4>
                            <div className="clisyc-filter-options-list">
                                {currentOptions.map(opt => {
                                    const isChecked = (selections[key] || []).includes(opt.value.toString());
                                    const slotCount = getSlotCount(key, opt.value.toString());
                                    
                                    return (
                                        <label key={opt.value} className={`clisyc-filter-option-label ${isChecked ? 'clisyc-option-selected' : ''}`}>
                                            <input
                                                type="checkbox"
                                                value={opt.value}
                                                checked={isChecked}
                                            ...
                                            {/* Slot count badge */}
                                            {slotCount !== null && (
                                                <span className={`clisyc-filter-slot-count ${slotCount === 0 ? 'clisyc-count-zero' : ''}`}>
                                                    {slotCount}
                                                </span>
                                            )}
                                        </label>
