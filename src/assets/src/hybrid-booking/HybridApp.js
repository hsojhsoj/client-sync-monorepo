...
const HybridApp = ({ displayOptions: propDisplayOptions = {} }) => {
    ...
    const [slotCounts, setSlotCounts] = useState({ total: 0, byFilter: {} });
    ...
    const appStyles = useMemo(() => {
        const styles = {};
        styles['--clisyc-available-bg'] = colorSettings?.available_bg || '#1f2937';
        styles['--clisyc-available-text'] = colorSettings?.available_text || '#ffffff';
        styles['--clisyc-unavailable-bg'] = colorSettings?.booked_bg || '#e5e7eb';
        styles['--clisyc-unavailable-text'] = colorSettings?.booked_text || '#4b5563';
        return styles;
    }, [colorSettings]);
    ...
    const generateDateRange = useCallback((startDate) => {
        ...
        if (startDate) {
            ...
            effectiveStartDate = new Date(nextAvailableDate + 'T00:00:00');
        } else {
    ...
    useEffect(() => {
        if (nextAvailableDate) {
            ...
            if (isMobileView) {
                ...
                visibleRangeEnd = new Date(visibleRangeStart.getTime() + 3 * 24 * 60 * 60 * 1000);
            } else {
                visibleRangeStart = new Date(today);
                ...
                visibleRangeEnd = new Date(visibleRangeStart.getTime() + 7 * 24 * 60 * 60 * 1000);
            }
    ...
    const handleFilterSelection = useCallback((key, values) => {
     ...
     * 
     * @returns {Promise<Date|null>} - First available date, or null if none
     */
    const findFirstAvailableInRange = async () => {
        ...
        try {
            const data = await apiFetch({ path: `/clisyc/v1/slots?${queryParams.toString()}` });
            ...
            availableEvents.sort((a, b) => new Date(a.start) - new Date(b.start));
    ...
    const handleSlotCountsUpdate = useCallback(async (counts) => {
        setSlotCounts(counts);
        
        // Check for no availability on first fetch (only once)
        if (!hasCheckedAvailability && !isCheckingBroaderAvailabilityRef.current) {
            ...
            if (counts.total === 0) {
                ...
                if (firstAvailable) {
                    ...
                    const formattedDate = firstAvailable.toLocaleDateString(undefined, {
                        weekday: 'long',
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric'
    ...
    }, [hasCheckedAvailability, selectedFilters, restNonce, generationLookahead, generateDateRange]);

    // Handle slot selection
    const handleSlotSelect = useCallback((event) => {
        const newSelectedSlot = {
            event,
            identifier: event.extendedProps?.slotIdentifier || event.slotIdentifier,
            durationMinutes: event.extendedProps?.durationMinutes || event.durationMinutes,
            displayTime: event.displayTime,
            fullDescription: event.fullDescription,
            start: event.start,
            dimensions: event.extendedProps?.dimensions || event.dimensions || {},
    ...
    return (
        <>
            ...
            {isSmartDateModalOpen && (
                <div className={`clisyc-modal-overlay ${textSizeClass}`} onClick={() => setIsSmartDateModalOpen(false)}>
                    <div className="clisyc-modal-content" onClick={e => e.stopPropagation()}>
                        ...
                        <p>
                            {decodeHTMLEntities(l10n?.jumpedAheadMessage) || "We've jumped ahead to the first available date for you."}
                        </p>
            ...
            <div className="clisyc-hybrid-booking-app clisyc-compact-booking-form-app" style={appStyles}>
                <div className={`clisyc-hybrid-layout ${!showFilters ? 'clisyc-hybrid-no-filters' : ''}`}>
                    ...
                    <div className="clisyc-hybrid-main-content">
                        ...
                        <div id="clisyc_hybrid_selected_slot_display" className={selectedSlot ? 'clisyc-hybrid-slot-selected' : 'none'}>
                            <span className="clisyc-hybrid-slot-label">{l10n?.selectedSlot || 'Selected Slot:'}</span>
                            <span id="clisyc_hybrid_selected_slot" className="clisyc-hybrid-slot-value">
                                {selectedSlot ? selectedSlot.displayTime : (l10n?.none || 'None')}
                            </span>
                        </div>

                        {/* Grid Header with Navigation */}
                        <div className="clisyc-hybrid-grid-header">
                            <button type="button" className="clisyc-hybrid-nav-button" onClick={() => navigateRange('prev')}>
                                {isMobileView ? '‹ Prev Day' : '‹ Previous 7 Days'}
                            </button>
                            <div className="clisyc-hybrid-date-range-display">
                                {dateRange.start && dateRange.end && (
                                    <span className="clisyc-hybrid-range-text">
                                        {dateRange.start.toLocaleDateString([], { month: 'short', day: 'numeric' })} - {new Date(dateRange.end - 86400000).toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric' })}
                                    </span>
                                )}
                            </div>
                            <button type="button" className="clisyc-hybrid-nav-button" onClick={() => navigateRange('next')}>
                                {isMobileView ? 'Next Day ›' : 'Next 7 Days ›'}
                            </button>
                        </div>
            ...
            {isConfirmationModalOpen && selectedSlot && (
                <div className="clisyc-hybrid-modal-overlay" onClick={() => setIsConfirmationModalOpen(false)}>
                    <div className="clisyc-hybrid-modal-content" onClick={e => e.stopPropagation()}>
                        <form method="post" id="clisyc_appointment_form" action="" noValidate>
                            <input type="hidden" name="_clisyc_frontend_booking_submit" value="1" />
                            <input type="hidden" name="booking_mode" value="slot" />
                            <input type="hidden" name="time_slot" value={selectedSlot.identifier || ''} />
                            <input type="hidden" name="appointment_duration_minutes" value={selectedSlot.durationMinutes || ''} />
                            <input type="hidden" name="appointment_date" value={selectedSlot.start ? new Date(selectedSlot.start).toISOString().split('T')[0] : ''} />
                            <input type="hidden" name="clisyc_appointment_nonce" value={formNonce} />

                            {selectedSlot.dimensions && typeof selectedSlot.dimensions === 'object' &&
                                Object.entries(selectedSlot.dimensions).map(([key, value]) => (
                            ...
                            {selectedSeats.length > 0 && (
                                <>
                                    ...
                                    <input type="hidden" name="seat_session_token" value={sessionStorage.getItem('clisyc_seat_session_token') || ''} />
                                </>
                            )}

                            <h3>{l10n?.confirmBooking || 'Confirm Your Booking'}</h3>
                            <div className="clisyc-confirmation-details">
                                <p><strong>You are booking:</strong></p>
                                <p>{selectedSlot.fullDescription || 'No description available'}</p>
                                <hr/>
                                <p><strong>Time:</strong> {selectedSlot.displayTime || 'No time available'}</p>
                            </div>

                            {/* Seat Map Picker — rendered when a venue is linked to this slot */}
                            {resolvedVenueId > 0 && selectedSlot.slotId > 0 && (
                                <SeatMapPicker
                            ...
                            <div className="clisyc-custom-fields-placeholder">
                                <p><em>Additional fields would appear here.</em></p>
                            </div>

                            <button type="submit" className="clisyc-submit-button button button-primary" style={{width: '100%', marginTop: '1em'}}>
                                {paymentRequired ? (l10n?.proceedToCheckout || 'Proceed to Checkout') : (l10n?.scheduleAppointment || 'Schedule Appointment')}
                            </button>
                            <button type="button" className="clisyc-hybrid-modal-close" onClick={() => setIsConfirmationModalOpen(false)} style={{marginTop: '10px'}}>
                                {l10n?.cancel || 'Cancel'}
                            </button>
                        </form>
                    </div>
