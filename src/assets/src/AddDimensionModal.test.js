...
describe('AddDimensionModal', () => {
  ...
  it('renders the modal with all form fields', () => {
    ...
    render(<AddDimensionModal onClose={mockOnClose} onSave={mockOnSave} />);

    // Assert: Check if the key elements are present in the document.
    expect(screen.getByRole('heading', { name: /Add New Dimension Type/i })).toBeInTheDocument();
    ...
    expect(screen.getByRole('button', { name: /Add Dimension Type/i })).toBeInTheDocument();
  });
  ...
  it('calls onSave with the correct data when the form is submitted', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /Add Dimension Type/i }));
  ...
  it('calls onClose when the cancel button is clicked', async () => {
    ...
    render(<AddDimensionModal onClose={mockOnClose} onSave={jest.fn()} />);
