...
function initReactApp() {
	...
	if ( rootEl ) {
		...
		if ( typeof window.clisycScheduleEditorData === 'undefined' ) {
			console.error(
				'[Client Sync Schedule Editor] Configuration data missing. Check wp_localize_script.'
			);
			rootEl.innerHTML =
				'<p style="color: red; text-align: center;"><strong>Error:</strong> Missing required configuration data. Cannot load editor.</p>';
			return;
		}
		...
		try {
			...
			console.error(
				'[Client Sync Schedule Editor] Critical render error:',
				error
			);
			rootEl.innerHTML =
				'<p style="color: red;">A critical error occurred while rendering the editor. Check the browser console for details.</p>';
		}
	}
}
