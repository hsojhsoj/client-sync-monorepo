...
const ScheduleEditorApp = () => {
    ...
    const initialScheduleJson = editorData.scheduleJson || '{"templates":{"A":{}}}';
    ...
    useEffect(() => {
        outputTextareaRef.current = document.getElementById('clisyc_schedule_json_output');
    }, []);
    ...
    const scheduleData = useMemo(() => {
        try {
            ...
            return (typeof parsed === 'object' && parsed !== null) ? parsed : { templates: { 'A': {} } };
        } catch (e) {
            console.error('[Schedule Editor] Error parsing schedule JSON:', e);
            return { templates: { 'A': {} } };
        }
    }, [scheduleJson]);
    ...
    const collectAndSaveAllCalendarData = useCallback(() => {
        ...
        if (!newSchedule.templates) newSchedule.templates = {};
        if (!newSchedule.templates[activeTemplateKey]) newSchedule.templates[activeTemplateKey] = {};
        
        // Loop through all calendar instances and collect their data
        Object.entries(calendarRefs.current).forEach(([dayIndex, calendar]) => {
            if (calendar) {
                ...
                if (events.length > 0) {
                    ...
                    newSchedule.templates[activeTemplateKey][dayIndex] = { slots };
                } else {
                    delete newSchedule.templates[activeTemplateKey][dayIndex];
                }
            }
        });
    ...
    const handleAddTemplate = () => {
        ...
        const newKey = String.fromCharCode(65 + Object.keys(scheduleData.templates || {}).length);
    ...
    const handleDeleteTemplate = (keyToDelete) => {
        if (!window.confirm(`Are you sure you want to delete "Week ${keyToDelete}"? This cannot be undone.`)) return;
        
        collectAndSaveAllCalendarData();
        const { [keyToDelete]: _, ...remainingTemplates } = scheduleData.templates;
        const newTemplates = {};
        Object.keys(remainingTemplates).sort().forEach((oldKey, index) => {
    ...
    useEffect(() => {
        ...
        const calendarEl = document.getElementById(`clisyc-schedule-day-calendar-${dayIndex}`);
        
        // Only create calendar if it doesn't exist yet
        if (calendarEl && !calendarRefs.current[dayIndex]) {
            const { calendarOptions } = editorData;
            const currentTemplateData = scheduleData.templates?.[activeTemplateKey]?.[dayIndex]?.slots || [];
            ...
            const calendar = new Calendar(calendarEl, {
                ...
                eventClick: (info) => {
                    if (info.jsEvent.target.classList.contains('clisyc-delete-event-btn')) {
    ...
    }, [activeDayIndex, activeTemplateKey, scheduleData, collectAndSaveAllCalendarData, editorData, weekdays]);
    ...
    const templateKeys = Object.keys(scheduleData.templates || {'A':{}}).sort();
        ...
        <div className="clisyc-weekly-schedule-container">
            <div className="clisyc-template-nav-container">
                {templateKeys.map(key => (
                    <span 
                        key={key} 
                        className={`clisyc-template-tab ${key === activeTemplateKey ? 'active' : ''}`} 
            ...
            <nav className="nav-tab-wrapper clisyc-schedule-day-tabs">
                {orderedWeekdays.map(({ index, name }) => {
                    const hasSlots = scheduleData.templates?.[activeTemplateKey]?.[index]?.slots?.length > 0;
                    return (
                        <a 
                            key={index} 
                            href="#" 
                            className={`nav-tab clisyc-day-tab ${index === activeDayIndex ? 'nav-tab-active' : ''} ${hasSlots ? 'has-slots' : 'no-slots'}`} 
                            onClick={(e) => { 
                                e.preventDefault(); 
                                handleDaySwitch(index);
                            }}
                        >
                            <span className="clisyc-day-tab-indicator"></span> {name}
                        </a>
                    );
                })}
            </nav>
            ...
            <div className="clisyc-schedule-calendars-wrapper">
                {orderedWeekdays.map(({ index }) => (
                    <div 
                        ...
                        style={{ display: index === activeDayIndex ? 'block' : 'none' }}
                    >
                        <div id={`clisyc-schedule-day-calendar-${index}`}></div>
                    </div>
                ))}
            </div>
        </div>
