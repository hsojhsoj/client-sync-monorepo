...
describe('FilterPanel', () => {
	...
	it('hides "Show booked slots" in date_range mode', () => {
		...
		expect(screen.queryByText('Show booked slots')).not.toBeInTheDocument();
	});
	...
	it('calls onToggleShowBooked when booked toggle is clicked', async () => {
		...
		const bookedLabel = screen.getByText('Show booked slots').closest('label');
	...
	it('renders collapsed state with hamburger icon only', () => {
		render(<FilterPanel {...defaultProps} isSidebarCollapsed={true} />);
	...
	it('shows reset button when filters are partially selected', () => {
		render(
			<FilterPanel
				{...defaultProps}
				selections={{ clisyc_service: ['1'], clisyc_practitioner: ['10', '11'] }}
			/>
		);
