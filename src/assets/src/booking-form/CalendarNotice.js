...
const CalendarNotice = ({
    ...
    const checkForOutOfRangeSlots = async (requestId) => {
        try {
            ...
            const [startHour, startMin] = calendarStartTime.split(':').map(Number);
            const [endHour, endMin] = calendarEndTime.split(':').map(Number);
            ...
            events.forEach(event => {
                ...
                if (!props.isBooked && !props.is_block) {
                    ...
                    if (eventTotalMinutes < calStartMinutes || eventTotalMinutes >= calEndMinutes) {
                        slotsOutsideRange++;
                    }
                }
            });
            ...
            if (slotsOutsideRange > 0 && slotsOutsideRange === totalAvailableSlots) {
                setNotice({
                    type: 'warning',
                    title: l10n.noticeOutOfRangeTitle || 'Slots Outside Calendar Hours',
                    message: l10n.noticeOutOfRangeMessage || 
                ...
                setNotice({
                    type: 'info',
                    title: l10n.noticeSomeOutOfRangeTitle || 'Some Slots Outside Calendar Hours',
                    message: l10n.noticeSomeOutOfRangeMessage || 
    ...
    return (
        <div className={`clisyc-calendar-notice clisyc-notice-${notice.type} clisyc-admin-notice`}>
            ...
            <div className="clisyc-notice-content">
                ...
                <div className="clisyc-notice-message">
                    {notice.message}
                </div>
            </div>
            <button 
