...
describe('BookingModal', () => {
	...
	it('has correct ARIA attributes', () => {
		...
		expect(dialog).toHaveAttribute('aria-labelledby', 'clisyc-booking-modal-title');
	});
	...
	it('calls onClose on backdrop click', async () => {
		...
		const overlay = container.querySelector('.clisyc-booking-modal-overlay');
	...
	it('renders custom fields when provided', () => {
		...
		expect(screen.getByLabelText('Additional Info')).toBeInTheDocument();
	});
	...
	it('shows invoice breakdown when basket has items', () => {
		const basket = {
			...
			items: [
				...
				{ label: 'Premium Room', price: 20, type: 'fee', meta: 'Add-on' },
			],
		};
	...
	it('shows price calculating state', () => {
		render(
			<BookingModal
				{...defaultProps}
				basket={{ total: 0, formatted: '', items: [{ label: 'x', price: 0 }] }}
	...
	it('shows reservation title in date_range mode', () => {
		...
		expect(screen.getByText('Confirm Your Reservation')).toBeInTheDocument();
