...
const App = () => {
    ...
    const handleToggleSidebar = useCallback(() => {
        dispatch({ type: ActionTypes.SET_IS_SIDEBAR_COLLAPSED, payload: !isSidebarCollapsed });
    }, [dispatch, isSidebarCollapsed]);

    // Toggle header collapsed/expanded
    const handleToggleHeader = useCallback(() => {
        dispatch({ type: ActionTypes.SET_IS_HEADER_COLLAPSED, payload: !isHeaderCollapsed });
    ...
    const handleMiniCalendarDateSelect = useCallback((date) => {
        dispatch({ type: ActionTypes.SET_MAIN_CALENDAR_DATE, payload: date });
    ...
    useEffect(() => {
        const timer = setInterval(() => {
            try {
                ...
                dispatch({ type: ActionTypes.SET_CURRENT_TIME, payload: now.toLocaleTimeString(undefined, options) });
            } catch (e) {
                dispatch({ type: ActionTypes.SET_CURRENT_TIME, payload: 'Invalid TZ' });
            }
        }, 1000);
    ...
    const fetchAllFilterOptions = useCallback(async () => {
        ...
        try {
            ...
            dispatch({ type: ActionTypes.SET_ALL_FILTER_OPTIONS, payload: response });
            ...
            if (preselectedServiceId) {
                ...
                if (isDateRangeMode && response[primaryDimKey]) {
                    ...
                    if (item) {
                        dispatch({ type: ActionTypes.SET_SELECTED_PRIMARY_ITEM, payload: item });
                    } else {
                        // Item not in filter options - create a minimal object
                        dispatch({ type: ActionTypes.SET_SELECTED_PRIMARY_ITEM, payload: {
            ...
            dispatch({ type: ActionTypes.SET_SELECTED_FILTERS, payload: initiallySelected });
        } catch (error) {
            console.error("Failed to fetch filter options:", error);
        } finally {
            dispatch({ type: ActionTypes.SET_IS_LOADING_FILTERS, payload: false });
        }
    }, [dispatch, restNonce, filterOrder, isDateRangeMode, preselectedServiceId, primaryDimKey, primaryItemName, dateRangeSettings]);
    ...
    useEffect(() => {
        ...
        if (!primaryId) {
            dispatch({ type: ActionTypes.SET_BASKET, payload: { total: 0, formatted: '', items: [] } });
            return;
        }
        ...
        const calculateTotal = async () => {
            dispatch({ type: ActionTypes.SET_IS_CALCULATING_PRICE, payload: true });
            ...
            if (selectedSeats && selectedSeats.length > 0) {
                params.append('selected_seats', JSON.stringify(selectedSeats));
            }
            ...
            try {
                ...
                if (response.success) {
                    dispatch({ type: ActionTypes.SET_BASKET, payload: {
                ...
                if (!abortController.signal.aborted) {
                    dispatch({ type: ActionTypes.SET_IS_CALCULATING_PRICE, payload: false });
                }
            }
        };
    ...
    const handleFilterSelection = useCallback((key, value) => {
        dispatch({ type: ActionTypes.UPDATE_SELECTED_FILTER, payload: { key, value } });

        // Clear selections when filters change
        dispatch({ type: ActionTypes.SET_SELECTED_SLOT, payload: null });
        dispatch({ type: ActionTypes.SET_SELECTED_DATE_RANGE, payload: null });
        ...
        if (isDateRangeMode && key === primaryDimKey && value) {
            ...
            dispatch({ type: ActionTypes.SET_SELECTED_PRIMARY_ITEM, payload: item || null });
        }
    ...
    const handleSlotSelect = useCallback((event) => {
        ...
        dispatch({ type: ActionTypes.SET_SELECTED_SLOT, payload: {
            event,
        ...
        dispatch({ type: ActionTypes.SET_IS_BOOKING_MODAL_OPEN, payload: true });
    }, [dispatch]);
    ...
    const handleDateRangeSelect = useCallback((rangeData) => {
        ...
        dispatch({ type: ActionTypes.SET_SELECTED_DATE_RANGE, payload: {
    ...
    const handleDeselect = useCallback(() => {
        dispatch({ type: ActionTypes.SET_SELECTED_SLOT, payload: null });
        dispatch({ type: ActionTypes.SET_SELECTED_DATE_RANGE, payload: null });
        dispatch({ type: ActionTypes.SET_IS_BOOKING_MODAL_OPEN, payload: false });
    }, [dispatch]);
    ...
    const handleFirstAvailableDate = useCallback((verifiedDate) => {
        ...
        if (verifiedDate === null) {
            dispatch({ type: ActionTypes.SET_IS_NO_AVAILABILITY_MODAL_OPEN, payload: true });
            return;
        }
        ...
        if (verifiedDateOnly.getTime() > today.getTime()) {
            ...
            dispatch({ type: ActionTypes.SET_SMART_DATE_MODAL_MESSAGE, payload: formattedDate });
            dispatch({ type: ActionTypes.SET_IS_SMART_DATE_MODAL_OPEN, payload: true });
        }
    ...
    const handleEventMouseEnter = useCallback((info) => {
        ...
        dispatch({ type: ActionTypes.SET_TOOLTIP, payload: { visible: true, x, y, content } });
    }, [dispatch]);

    const handleEventMouseLeave = useCallback(() => {
        dispatch({ type: ActionTypes.SET_TOOLTIP, payload: { visible: false, x: 0, y: 0, content: null } });
    }, [dispatch]);
    ...
    return (
        <div className={`clisyc-booking-form-app ${isDateRangeMode ? 'clisyc-date-range-mode' : 'clisyc-slot-mode'} ${preselectedServiceId ? 'clisyc-preselected' : ''}`}>
            ...
            {isSmartDateModalOpen && !isDateRangeMode && (
                <div className={`clisyc-modal-overlay ${textSizeClass}`} onClick={() => dispatch({ type: ActionTypes.SET_IS_SMART_DATE_MODAL_OPEN, payload: false })}>
                    <div className="clisyc-modal-content" onClick={e => e.stopPropagation()}>
                        ...
                        <button 
                            ...
                            onClick={() => dispatch({ type: ActionTypes.SET_IS_SMART_DATE_MODAL_OPEN, payload: false })}
                        >
            ...
            {isNoAvailabilityModalOpen && !isDateRangeMode && (
                <div className={`clisyc-modal-overlay ${textSizeClass}`} onClick={() => dispatch({ type: ActionTypes.SET_IS_NO_AVAILABILITY_MODAL_OPEN, payload: false })}>
                    <div className="clisyc-modal-content" onClick={e => e.stopPropagation()}>
                        <div className="clisyc-modal-icon">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                <rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect>
                        ...
                        <div className="clisyc-modal-buttons">
                            ...
                            <button 
                                ...
                                onClick={() => dispatch({ type: ActionTypes.SET_IS_NO_AVAILABILITY_MODAL_OPEN, payload: false })}
                            >
            ...
            <BookingModal
                ...
                onSeatsChange={(seats) => dispatch({ type: ActionTypes.SET_SELECTED_SEATS, payload: seats })}
            ...
            <div className={`clisyc-faceted-search-layout ${!showFilters ? 'clisyc-no-sidebar' : ''} ${isSidebarCollapsed ? 'clisyc-sidebar-collapsed' : ''}`}>
                ...
                {showFilters && (
                    <FilterPanel
                        ...
                        onToggleShowBooked={showBookedToggle ? (val) => dispatch({ type: ActionTypes.SET_SHOW_BOOKED, payload: val }) : null}
                                        ...
                                        <div className="clisyc-timezone-filter-wrapper">
                                            ...
                                            <div className="clisyc-timezone-selector">
                                                <select 
                                                    ...
                                                    value={timezone} 
                                                    onChange={(e) => dispatch({ type: ActionTypes.SET_TIMEZONE, payload: e.target.value })}
                    ...
                    {!isDateRangeMode && selectedSlot && (
                        <div className="clisyc-selected-slot-banner">
                            ...
                            <button 
                                ...
                                onClick={() => dispatch({ type: ActionTypes.SET_IS_BOOKING_MODAL_OPEN, payload: true })}
                            >
                                Complete Booking
                            </button>
                        </div>
                        ...
                        <>
                            ...
                            {(primaryDimValue || preselectedServiceId) && (
                                <div className="clisyc-date-range-tz-bar">
                                    <span className="dashicons dashicons-admin-site" aria-hidden="true"></span>
                                    <span>{timezone === 'local' ? (l10n?.localTime || 'Local Time') : timezone.replace('_', ' ').split('/').pop()}</span>
                                    <span className="clisyc-timezone-offset-badge">{getTimezoneOffsetLabel(timezone)}</span>
                                </div>
                            )}
