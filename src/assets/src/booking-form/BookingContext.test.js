...
const TestConsumer = ({ actionToDispatch } = {}) => {
	...
	return (
		<div>
			...
			<span data-testid="showBooked">{state.showBooked.toString()}</span>
...
describe('BookingContext', () => {
	it('provides default state through the context', () => {
		...
		expect(screen.getByTestId('isLoadingFilters')).toHaveTextContent('true');
		expect(screen.getByTestId('isBookingModalOpen')).toHaveTextContent('false');
		expect(screen.getByTestId('showBooked')).toHaveTextContent('false');
	...
	it('dispatches SET_IS_BOOKING_MODAL_OPEN', () => {
		const Wrapper = () => {
			...
			return (
				<>
					<span data-testid="open">{state.isBookingModalOpen.toString()}</span>
					<button
						onClick={() =>
							dispatch({
					...
					<button
						onClick={() =>
							dispatch({
								...
								payload: { key: 'clisyc_service', value: ['1', '2'] },
							})
						}
					>
						Select
		...
		const Wrapper = () => {
			...
			return (
				<>
					...
					<button
						onClick={() => dispatch({ type: ActionTypes.RESET_SELECTIONS })}
					>
						Reset
					</button>
				</>
	...
	it('dispatches SET_SEAT_HOLD_EXPIRY', () => {
		...
		expect(screen.getByTestId('expiry')).toHaveTextContent('1700000000');
	});
	...
	it('RESET_SELECTIONS clears selectedSeats', () => {
		...
		expect(JSON.parse(screen.getByTestId('seats').textContent)).toHaveLength(2);

		act(() => {
			screen.getByText('Reset All').click();
		});
		expect(JSON.parse(screen.getByTestId('seats').textContent)).toHaveLength(0);
	});
		...
		const Wrapper = () => {
			...
			return (
				<>
					...
					<button
						onClick={() =>
							dispatch({ type: 'UNKNOWN_ACTION', payload: 'test' })
						}
					>
						Unknown
					</button>
