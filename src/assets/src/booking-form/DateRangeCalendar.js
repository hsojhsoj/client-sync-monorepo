...
const DateRangeCalendar = ({ 
    ...
    const [selectedRange, setSelectedRange] = useState({ start: null, end: null, nights: 0 });
    ...
    const fetchBookedDates = useCallback(async (start, end) => {
        if (!dimensionId) {
            setIsLoading(false);
            return;
        }
    ...
    const isDateBooked = useCallback((dateStr) => {
        return bookedDates.includes(dateStr);
    }, [bookedDates]);

    // Check if any date in a range is booked
    const isRangeBooked = useCallback((startStr, endStr) => {
    ...
    const calculateTotal = useCallback((nights) => {
        ...
        const calendar = new Calendar(calendarRef.current, {
            ...
            select: (info) => {
                ...
                if (isRangeBooked(startDate, endDate)) {
                    calendar.unselect();
                    setError('Some dates in your selection are unavailable. Please choose different dates.');
                ...
                if (nights > maxStay) {
                    calendar.unselect();
                    setError(`Maximum stay is ${maxStay} night${maxStay !== 1 ? 's' : ''}.`);
                    setTimeout(() => setError(null), 4000);
                    return;
                }
            ...
            datesSet: (info) => {
                ...
                if (currentViewRange.end && viewEnd > currentViewRange.end) {
                    ...
                    setCurrentViewRange(prev => ({ ...prev, end: newEnd }));
    ...
    }, [bookedDates, minStay, maxStay, isRangeBooked, isDateBooked, calculateTotal, onDateRangeSelect]);
    ...
    return (
        <div className="clisyc-date-range-calendar">
                ...
                <div className="clisyc-date-range-header">
                    ...
                    {pricePerNight > 0 && (
                        <div className="clisyc-price-per-night">
                            {currencySymbol}{pricePerNight.toFixed(2)} / night
                        </div>
                    )}
                </div>
            )}
            ...
            <div className="clisyc-date-range-instructions">
                ...
                {minStay > 1 && <span className="clisyc-min-stay-note"> (min. {minStay} nights)</span>}
            </div>
                    ...
                    <div className="clisyc-range-details">
                        <div className="clisyc-range-dates">
                            <div className="clisyc-range-date">
                                <span className="clisyc-range-label">Check-in</span>
                                <span className="clisyc-range-value">{formatDate(selectedRange.start)}</span>
                            </div>
                            <div className="clisyc-range-arrow">→</div>
                            <div className="clisyc-range-date">
                                <span className="clisyc-range-label">Check-out</span>
                                <span className="clisyc-range-value">{formatDate(selectedRange.end)}</span>
                            </div>
                        </div>
                        <div className="clisyc-range-nights">
                            <strong>{selectedRange.nights}</strong> night{selectedRange.nights !== 1 ? 's' : ''}
                        </div>
                        {selectedRange.total && (
                            <div className="clisyc-range-total">
                                Total: <strong>{currencySymbol}{selectedRange.total}</strong>
                            </div>
                        )}
                    </div>
                    <button 
