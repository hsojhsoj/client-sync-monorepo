...
const EventTooltip = React.memo( ( { visible, x, y, content } ) => {
	...
	const adjustedY = Math.max(
		10,
		Math.min( y - 60, window.innerHeight - 180 )
	);
					...
					<div className="clisyc-event-tooltip-title">
						{ content.service && (
							<span
								...
								style={ {
									backgroundColor: content.serviceColor,
								} }
							/>
						) }
						{ content.title }
				...
				{ content.practitioner && (
					<div className="clisyc-event-tooltip-row">
						<span
							className="clisyc-event-tooltip-dot"
							style={ {
								backgroundColor: content.practitionerColor,
							} }
						/>
						<span className="clisyc-event-tooltip-label">
							{ content.practitioner }
						</span>
					</div>
				) }
