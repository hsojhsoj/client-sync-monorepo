...
describe( 'AnalogClock', () => {
	...
	it( 'renders clock container with hands and center dot', () => {
		...
		expect(
			container.querySelector( '.clisyc-clock-icon-container' )
		).toBeInTheDocument();
		expect(
			container.querySelector( '.clisyc-clock-hand-hour' )
		).toBeInTheDocument();
		expect(
			container.querySelector( '.clisyc-clock-hand-minute' )
		).toBeInTheDocument();
		expect(
			container.querySelector( '.clisyc-clock-center-dot' )
		).toBeInTheDocument();
	} );
	...
	it( 'renders with local timezone by default', () => {
		...
		expect( hourHand.style.transform ).toMatch(
			/rotate\(\d+(\.\d+)?deg\)/
		);
	} );
	...
	it( 'falls back to local time on invalid timezone', () => {
		const { container } = render(
			<AnalogClock timezone="Invalid/Timezone" />
		);
		const hourHand = container.querySelector( '.clisyc-clock-hand-hour' );
