...
describe( 'MiniCalendar', () => {
	...
	it( 'renders the calendar grid', () => {
		const { container } = render(
			<MiniCalendar currentDate={ fixedDate } />
		);

		expect(
			container.querySelector( '.clisyc-mini-calendar' )
		).toBeInTheDocument();
		expect(
			container.querySelector( '.clisyc-mini-cal-grid' )
		).toBeInTheDocument();
	} );
