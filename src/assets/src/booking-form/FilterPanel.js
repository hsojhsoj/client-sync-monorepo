...
const FilterPanel = React.memo(
	( {
		...
		const { availabilityDimensions, filterOrder, l10n, currencySymbol } =
		...
		const hasActiveFilters = useMemo(
			() =>
				Object.entries( selections ).some( ( [ key, val ] ) => {
					const totalOptions = options[ key ]?.length || 0;
					return (
						Array.isArray( val ) &&
						val.length > 0 &&
						val.length < totalOptions
					);
				} ),
			[ selections, options ]
		);
		...
		const ChevronIcon = ( { isExpanded } ) => (
			<svg
				className={ `clisyc-sidebar-chevron ${
					isExpanded ? 'clisyc-sidebar-chevron--expanded' : ''
				}` }
				width="16"
				height="16"
				viewBox="0 0 24 24"
		...
		return (
			<aside
				className={ `clisyc-sidebar ${
					isSidebarCollapsed ? 'clisyc-sidebar--collapsed' : ''
				} ${ isLoading ? 'clisyc-sidebar--loading' : '' } ${
					isDateRangeMode ? 'clisyc-sidebar--date-range' : ''
				}` }
			>
					...
					<div className="clisyc-sidebar-inner">
						...
						<nav className="clisyc-sidebar-nav">
							{ filterOrder?.map( ( key ) => {
								...
								const isPrimaryDimension =
									key === primaryDimKey;
								if ( isDateRangeMode && ! isPrimaryDimension ) {
									return null;
								}
								...
								return (
									<div
										key={ key }
										className={ `clisyc-sidebar-group ${
											isPrimaryDimension
												? 'clisyc-sidebar-group--primary'
												: ''
										}` }
									>
										...
										>
											...
											{ activeCount && (
												<span className="clisyc-sidebar-filter-badge">
													{ activeCount }
												</span>
											) }
										</button>
										...
										<div
											className={ `clisyc-sidebar-group-content ${
												isExpanded
													? 'clisyc-sidebar-group-content--expanded'
													: ''
											}` }
										>
											{ currentOptions ? (
												currentOptions.map( ( opt ) => {
													...
													return (
														<label
															key={ opt.value }
															className={ `clisyc-sidebar-item ${
																isSelected
																	? 'clisyc-sidebar-item--selected'
																	: ''
															}` }
														>
															<input
							...
							{ ! isDateRangeMode && onToggleShowBooked && (
								<div className="clisyc-sidebar-group clisyc-sidebar-group--options">
									<button
										...
										className="clisyc-sidebar-group-header"
										onClick={ () =>
											toggleGroup( 'viewOptions' )
										}
										aria-expanded={
											expandedGroups.viewOptions
										}
									>
										<ChevronIcon
											isExpanded={
						...
						{ isDateRangeMode && (
							<div className="clisyc-sidebar-info">
								...
								<span>
									{ l10n?.dateRangeInfo ||
										'Select an option to view availability' }
								</span>
							</div>
						) }
					</div>
