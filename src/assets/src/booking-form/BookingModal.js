...
const BookingModal = ({
    ...
    const { availabilityDimensions, filterOrder, wc_integration_enabled, stripe_integration_enabled, waitlist_enabled, proRecurringEnabled, restRootUrl, restNonce } = window.clisycBookingFormData || {};
    ...
    const paymentRequired = Boolean(wc_integration_enabled || stripe_integration_enabled);
    ...
    const handleSubmit = async (e) => {
        ...
        if (customFieldDefinitions && customFieldDefinitions.length > 0) {
            const missingFields = customFieldDefinitions
                .filter(field => field.required && !customFieldValues[field.key]?.toString().trim())
        ...
        if (isRecurring && proRecurringEnabled && !isDateRangeMode && selectedSlot) {
            try {
                ...
                const seriesResponse = await fetch(
                    `${restRootUrl || '/wp-json/'}client-sync-pro/v1/appointments/series`,
                    {
                        method: 'POST',
                        headers: {
                ...
                if (seriesResult.success || seriesResponse.status === 201) {
                    ...
                    setTimeout(() => {
                        if (seriesResult.redirect_url) {
                            ...
                            const redirectUrl = buildRedirectUrl(successPageUrl, seriesResult.appointment_ids?.[0]);
                            if (redirectUrl) {
                    ...
                    setErrorMessage(seriesResult.message || 'Failed to create recurring series.');
                    setIsSubmitting(false);
                }
            } catch (error) {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.data?.code === 'slot_unavailable' && result.data?.waitlist_available && waitlist_enabled) {
                    setWaitlistOffer({
    ...
    const handleJoinWaitlist = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                setWaitlistOffer(prev => ({ ...prev, position: result.data?.position || prev?.position }));
            } else {
                setSubmitStatus('error');
                setErrorMessage(result.data?.message || 'Could not join the waitlist.');
            }
        } catch (error) {
            setSubmitStatus('error');
    ...
    return (
        <div className="clisyc-booking-modal-overlay" onClick={handleBackdropClick}>
            <div className="clisyc-booking-modal" ref={modalRef} role="dialog" aria-modal="true" aria-labelledby="clisyc-booking-modal-title" tabIndex="-1">
                ...
                {waitlistOffer && !submitStatus ? (
                    <div className="clisyc-booking-modal-waitlist-offer">
                        <div className="clisyc-waitlist-icon" style={{ fontSize: '2.5rem', textAlign: 'center', marginBottom: '0.75rem' }}>⏳</div>
                        <h2 style={{ textAlign: 'center', margin: '0 0 0.5rem' }}>This Slot is Full</h2>
                        <p style={{ textAlign: 'center', color: '#6b7280', margin: '0 0 1.25rem' }}>
                            Would you like to join the waitlist? You'll be notified automatically if a spot opens up.
                        </p>
                        <div style={{ background: '#eff6ff', borderRadius: '8px', padding: '1rem', textAlign: 'center', marginBottom: '1.25rem' }}>
                            <div style={{ fontSize: '0.85rem', color: '#6b7280' }}>Your waitlist position</div>
                            <div style={{ fontSize: '1.75rem', fontWeight: '700', color: '#1e40af' }}>#{waitlistOffer.position}</div>
                        </div>
                        <div className="clisyc-booking-modal-actions">
                            <button type="button" className="clisyc-booking-modal-btn-cancel" onClick={() => { setWaitlistOffer(null); onClose(); }}>
                                No Thanks
                            </button>
                            <button
                                type="button"
                            ...
                            >
                                {isJoiningWaitlist ? (
                                    <><span className="clisyc-btn-spinner"></span> Joining...</>
                                ) : (
                                    'Join Waitlist'
                                )}
                            </button>
                    ...
                    <div className="clisyc-booking-modal-success">
                        ...
                        <p>You're #{waitlistOffer?.position} on the waitlist. We'll notify you as soon as a spot opens up.</p>
                        <div className="clisyc-booking-modal-actions" style={{ marginTop: '1.5rem', justifyContent: 'center' }}>
                            <button type="button" className="clisyc-booking-modal-btn-confirm" onClick={onClose}>
                                Got It
                            </button>
                        </div>
                    </div>
                    ...
                    <>
                        ...
                        <div className="clisyc-booking-modal-header">
                            <h2 id="clisyc-booking-modal-title" className="clisyc-booking-modal-title">
                        ...
                        {effectiveVenueId > 0 && selectedSlot && (
                            <SeatMapPicker
                                ...
                                slotId={selectedSlot.event?.extendedProps?.slot_id || 0}
