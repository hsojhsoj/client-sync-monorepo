...
const getMapImageUrl = (propUrl) => {
    ...
    if (phpData.timezoneMapUrl) return phpData.timezoneMapUrl;
    if (phpData.pluginUrl) return `${phpData.pluginUrl}assets/images/timezone-map.png`;
...
const TimezoneMap = ({ 
    ...
    const utcOffset = useMemo(() => getUtcOffset(selectedTimezone), [selectedTimezone]);
    const highlightPosition = useMemo(() => getPositionForOffset(utcOffset), [utcOffset]);
    const offsetLabel = useMemo(() => getOffsetLabel(utcOffset), [utcOffset]);
    const imageUrl = useMemo(() => getMapImageUrl(mapImageUrl), [mapImageUrl]);
        ...
        <div className="clisyc-timezone-map-wrapper">
            ...
            >
                <img 
                    ...
                    onError={(e) => {
                        ...
                        console.warn('TimezoneMap: Failed to load map image from', imageUrl);
                    }}
                />
            ...
            {showLabel && (
                <div className="clisyc-timezone-map-label">
                    ...
                    <span className="clisyc-map-offset-label">{offsetLabel}</span>
                </div>
            )}
        </div>
    );
