...
const CalendarView = ({ 
    ...
    const {
        startOfWeek = 0,
        enabledViews = ['dayGridMonth', 'timeGridWeek', 'timeGridDay', 'listWeek'],
    ...
    useEffect(() => {
        if (calendarInstanceRef.current) {
            ...
            document.querySelectorAll('.fc-event.clisyc-slot-selected').forEach(el => {
            ...
            if (selectedSlotId) {
                document.querySelectorAll('.fc-event').forEach(el => {
                    const event = calendarInstanceRef.current.getEventById(selectedSlotId);
                    if (event && el.getAttribute('data-slot-id') === selectedSlotId) {
    ...
    const reportFirstAvailableDate = async (viewEvents) => {
        ...
        if (availableInView.length > 0) {
            availableInView.sort((a, b) => new Date(a.start) - new Date(b.start));
        ...
        const firstAvailable = await findFirstAvailableInRange(selectedFiltersRef.current);
    ...
    useEffect(() => {
        if (calendarRef.current && !calendarInstanceRef.current) {
            ...
            const calendar = new Calendar(calendarRef.current, {
                ...
                initialView: isMobile ? 'timeGridDay' : (initialView || 'timeGridWeek'),
                height: 'auto',
                nowIndicator: true,
                editable: false,
                selectable: false,
                ...
                events: (fetchInfo, successCallback, failureCallback) => {
                    ...
                    apiFetch({ path: `/clisyc/v1/slots?${queryParams.toString()}` })
                        .then(data => {
                            const rawEvents = data.events || [];
                            
                            // Count available (bookable) slots
                            const availableCount = rawEvents.filter(e => 
                                e.extendedProps?.status === 'available'
                            ).length;
                ...
                eventContent: (arg) => {
                    ...
                    // CSS color validator
                    const safeColor = (c) => (c && /^(#[0-9a-f]{3,8}|rgb|hsl|transparent|[a-z]+)/i.test(c)) ? c : 'transparent';

                    const timeHtml = `<div class="fc-event-time">${esc(arg.timeText)}</div>`;
                    let detailsHtml = '';
                    ...
                    if (isTimeGridView) {
                        ...
                        filterOrder.forEach(slug => {
                            ...
                            if (detailLabel) {
                                const dotHtml = detailColor ? `<span class="clisyc-event-color-dot" style="background-color: ${safeColor(detailColor)};"></span>` : '';
                                compactDetails.push(`<span class="clisyc-event-compact-item">${dotHtml}<span class="clisyc-event-detail-label">${esc(detailLabel)}</span></span>`);
                            }
                        });
                        if (compactDetails.length > 0) {
                            detailsHtml = `<div class="clisyc-event-details-compact">${compactDetails.join('')}</div>`;
                        }
                    } else {
                        const fullDetails = [];
                        filterOrder.forEach(slug => {
                            ...
                            if (detailLabel) {
                                const dotHtml = detailColor ? `<span class="clisyc-event-color-dot" style="background-color: ${safeColor(detailColor)};"></span>` : '';
                                fullDetails.push(`<div class="clisyc-event-detail-item">${dotHtml}<span class="clisyc-event-detail-label">${esc(detailLabel)}</span></div>`);
                            }
                        });
                        if (fullDetails.length > 0) {
                            detailsHtml = `<div class="clisyc-event-details-stack">${fullDetails.join('')}</div>`;
                        }
                    }
                    ...
                    if (capacity > 1 && spotsLeft > 0 && spotsLeft <= Math.max(2, Math.ceil(capacity * 0.25))) {
                        urgencyHtml = `<span class="clisyc-urgency-badge">Only ${esc(String(spotsLeft))} left</span>`;
                    }

                    return {
                        html: `
                            <div class="fc-event-main-frame" data-slot-id="${esc(props.slotIdentifier || '')}">
                                ${timeHtml}
                                ${detailsHtml}
                                ${urgencyHtml}
                            </div>
                ...
                eventDidMount: (info) => {
                    ...
                    el.setAttribute('data-slot-id', props.slotIdentifier || '');
                    
                    // Calculate event duration in minutes
                    const eventDurationMinutes = props.durationMinutes || 
                        (info.event.end && info.event.start 
                            ? Math.round((info.event.end - info.event.start) / 60000) 
                            : 30);
                    el.setAttribute('data-duration', eventDurationMinutes.toString());
                    ...
                    el.style.setProperty('--event-service-color', serviceColor);
                    ...
                    if (info.view.type.includes('timeGrid') && info.event.start && info.event.end) {
                        ...
                        if (harness && harness.classList.contains('fc-timegrid-event-harness')) {
                            ...
                            const actualSlotHeight = slotEl.getBoundingClientRect().height;
                            ...
                            const slotDurationStr = customSlotDuration || '00:30:00';
                            const slotParts = slotDurationStr.split(':');
                            const slotDurationMinutes = (parseInt(slotParts[0]) * 60) + parseInt(slotParts[1]);
                            
                            // Calculate actual pixels per minute from rendered slot
                            const actualPixelsPerMinute = actualSlotHeight / slotDurationMinutes;
                            
                            // Get the slot min time (start of day column)
                            const slotMinStr = fcOptions.slotMinTime || '08:00';
                            const [minHour, minMinute] = slotMinStr.split(':').map(Number);
                            const slotMinTotalMinutes = minHour * 60 + (minMinute || 0);
                            ...
                            const minutesFromTop = eventStartMinutes - slotMinTotalMinutes;
                            
                            // Calculate pixel position and height using ACTUAL rendered values
                            const topPx = minutesFromTop * actualPixelsPerMinute;
                            const heightPx = eventDurationMinutes * actualPixelsPerMinute;
                    ...
                    if (props.isBooked) {
                        ...
                        if (selectedSlotId && props.slotIdentifier === selectedSlotId) {
                            el.classList.add('clisyc-slot-selected');
                        }
                    } else {
                ...
                eventMouseEnter: (info) => {
                    ...
                    const tooltipInfo = {
                        event: {
                            ...
                            extendedProps: {
                                serviceName: details.service_name || info.event.title,
                                serviceColor: details.service_color || '#1a73e8',
                                practitionerName: details.practitioner_name || '',
                                practitionerColor: details.practitioner_color || '#4285f4',
            ...
            if (calendarEl && typeof ResizeObserver !== 'undefined') {
                const resizeObserver = new ResizeObserver(() => {
                    if (!calendarInstanceRef.current || layoutFixAppliedRef.current) return;

                    const slot = calendarEl.querySelector('.fc-timegrid-slot');
                    const slotHeight = slot ? slot.getBoundingClientRect().height : 0;
    ...
    return (
        <div className="clisyc-calendar-section">
            <div className="clisyc-calendar-wrapper">
                {isLoading && (
                    <div className="clisyc-loading-overlay" style={{
                        position: 'absolute',
                        top: 0,
                        left: 0,
                        right: 0,
                        ...
                        <div style={{
                            ...
                            <div style={{ width: '48px', height: '48px', flexShrink: 0 }}>
                                <svg viewBox="0 0 50 50" style={{ 
                                    width: '100%', 
                                    height: '100%',
                                    animation: 'clisyc-spinner-rotate 1.5s linear infinite'
                                }}>
                                    <circle cx="25" cy="25" r="20" fill="none" strokeWidth="4" stroke="#e5e7eb"/>
                                    <circle cx="25" cy="25" r="20" fill="none" strokeWidth="4" stroke="#3b82f6"
                                        strokeLinecap="round"
                                        strokeDasharray="80, 200"
                                        style={{ animation: 'clisyc-spinner-dash 1.5s ease-in-out infinite' }}
                                    />
                                </svg>
                            </div>
                            {/* Text */}
