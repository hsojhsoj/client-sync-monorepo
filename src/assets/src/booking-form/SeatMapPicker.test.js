...
describe( 'SeatMapPicker', () => {
	...
	it( 'renders section overview when layout has multiple sections', async () => {
		...
		await waitFor( () => {
			expect(
				screen.getByText( /Select a section/i )
			).toBeInTheDocument();
		} );
	...
	it( 'shows selected seat count', async () => {
		...
		await waitFor( () => {
			expect(
				screen.getByText( /2 seats selected/i )
			).toBeInTheDocument();
		} );
	} );
	...
	it( 'shows singular "seat selected" for single selection', async () => {
		...
		await waitFor( () => {
			expect(
				screen.getByText( /1 seat selected/i )
			).toBeInTheDocument();
		} );
	} );
