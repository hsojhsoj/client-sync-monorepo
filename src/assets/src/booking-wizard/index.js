...
const SpinnerIcon = () => (
    <svg width="20" height="20" viewBox="0 0 20 20" className="clisyc-wizard-spinner">
            ...
            {steps.map((step, index) => {
                ...
                return (
                    <React.Fragment key={step.id}>
                        <div 
                            className={`clisyc-wizard-step ${isCompleted ? 'completed' : ''} ${isCurrent ? 'current' : ''} ${isClickable ? 'clickable' : ''}`}
                            onClick={() => isClickable && onStepClick(index)}
                        >
                            <div className="clisyc-wizard-step-number">
                                {isCompleted ? <CheckIcon /> : index + 1}
                            </div>
                            <div className="clisyc-wizard-step-label">{step.label}</div>
                        </div>
                        {index < steps.length - 1 && (
                            <div className={`clisyc-wizard-step-connector ${isCompleted ? 'completed' : ''}`} />
                        )}
                    </React.Fragment>
                );
            })}
...
const DimensionStep = ({ dimension, items, selectedItem, onSelect, previousSelections }) => {
    ...
    const availableItems = useMemo(() => {
        ...
        return items.filter(item => {
            ...
            if (!item.compatibleWith || Object.keys(item.compatibleWith).length === 0) {
                return true; // No restrictions defined, show item
            }
            
            // Check compatibility with each previous selection
            for (const [dimKey, selectedId] of Object.entries(previousSelections)) {
                ...
                const isCompatible = compatList.some(id => Number(id) === selectedIdNum);
    ...
    if (!availableItems || availableItems.length === 0) {
        return (
            <div className="clisyc-wizard-step-content">
                <h3 className="clisyc-wizard-step-title">Select {singularLabel}</h3>
                <div className="clisyc-wizard-empty">
                    No {pluralLabel.toLowerCase()} available for your current selections.
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="clisyc-wizard-step-content">
            ...
            <p className="clisyc-wizard-step-subtitle">
                {dimension.selection_description || `Choose your preferred ${singularLabel.toLowerCase()}`}
            </p>
            ...
            <div className="clisyc-wizard-dimension-grid">
                {availableItems.map(item => (
                    <div 
                        key={item.id}
                        className={`clisyc-wizard-dimension-card ${selectedItem?.id === item.id ? 'selected' : ''}`}
                    ...
                    >
                        ...
                        <div className="clisyc-wizard-dimension-content">
                            <h4 className="clisyc-wizard-dimension-name">{item.name}</h4>
                            {item.subtitle && (
                                <span className="clisyc-wizard-dimension-subtitle">{item.subtitle}</span>
                            )}
                            {item.duration && (
                            ...
                            {item.price && (
                                <span className="clisyc-wizard-dimension-meta">{item.price}</span>
                            )}
                            {item.description && (
                                <p className="clisyc-wizard-dimension-description">{item.description}</p>
                            )}
                        </div>
                        {selectedItem?.id === item.id && (
                            <div className="clisyc-wizard-dimension-check"><CheckIcon /></div>
                        )}
                    </div>
                ))}
            </div>
...
// Time Selection Step
const TimeStep = ({ timeSlots, selectedTime, onSelect, isLoading, selectedDate }) => {
    if (isLoading) {
        return (
            <div className="clisyc-wizard-step-content">
                ...
                {Object.entries(groupedSlots).map(([key, group]) => {
                    ...
                    return (
                        <div key={key} className="clisyc-wizard-time-group">
                            <h4 className="clisyc-wizard-time-group-label">{group.label}</h4>
                            <div className="clisyc-wizard-time-grid">
                                {group.slots.map(slot => (
                                    <button
                                        key={slot.id || slot.time}
                                        type="button"
                                        className={`clisyc-wizard-time-slot ${selectedTime?.time === slot.time ? 'selected' : ''}`}
...
const DetailsStep = ({ details, onChange, errors, currentUser }) => {
    ...
    return (
        <div className="clisyc-wizard-step-content">
            ...
            <p className="clisyc-wizard-step-subtitle">Please provide your contact information</p>
            ...
            <div className="clisyc-wizard-form">
                <div className="clisyc-wizard-form-row">
                    <div className="clisyc-wizard-form-field">
                        ...
                        />
                        {errors?.firstName && <span className="clisyc-wizard-field-error">{errors.firstName}</span>}
                    </div>
                    <div className="clisyc-wizard-form-field">
                        ...
                        />
                        {errors?.lastName && <span className="clisyc-wizard-field-error">{errors.lastName}</span>}
                    </div>
                </div>
                ...
                <div className="clisyc-wizard-form-field">
                    ...
                    />
                    {errors?.email && <span className="clisyc-wizard-field-error">{errors.email}</span>}
                </div>
...
const ConfirmationStep = ({ dimensions, selections, date, time, details, isSubmitting, error }) => {
    const formatTime = (timeStr) => {
        if (!timeStr) return '';
        const [hours, minutes] = timeStr.split(':');
        const hour = parseInt(hours);
    ...
    return (
        <div className="clisyc-wizard-step-content">
            ...
            <p className="clisyc-wizard-step-subtitle">Please confirm all details are correct</p>
            
            {error && (
                <div className="clisyc-wizard-error-message">
                    {error}
                </div>
            )}
            ...
            <div className="clisyc-wizard-summary">
                ...
                {dimensions.map(dim => {
                    ...
                    return (
                        <div key={dim.key} className="clisyc-wizard-summary-item">
                            <div 
                            ...
                            <div className="clisyc-wizard-summary-content">
                                <span className="clisyc-wizard-summary-label">{dim.singular_label}</span>
                                <span className="clisyc-wizard-summary-value">{selected.name}</span>
                                {selected.duration && (
                                    <span className="clisyc-wizard-summary-detail">{selected.duration} minutes</span>
                                )}
                            </div>
                        </div>
                    );
                ...
                <div className="clisyc-wizard-summary-item">
                    ...
                    <div className="clisyc-wizard-summary-content">
                        <span className="clisyc-wizard-summary-label">Date & Time</span>
                        ...
                        <span className="clisyc-wizard-summary-detail">{formatTime(time?.time)}</span>
                    </div>
                </div>
                ...
                <div className="clisyc-wizard-summary-item">
                    ...
                    <div className="clisyc-wizard-summary-content">
                        <span className="clisyc-wizard-summary-label">Your Information</span>
                        <span className="clisyc-wizard-summary-value">{details?.firstName} {details?.lastName}</span>
                        <span className="clisyc-wizard-summary-detail">{details?.email}</span>
                        {details?.phone && <span className="clisyc-wizard-summary-detail">{details.phone}</span>}
                    </div>
                </div>
                
                {details?.notes && (
                    <div className="clisyc-wizard-summary-notes">
                        <span className="clisyc-wizard-summary-label">Notes</span>
                        <p>{details.notes}</p>
                    </div>
                )}
            </div>
...
// Success Screen
const SuccessScreen = ({ dimensions, selections, date, time, details, confirmationNumber }) => {
    ...
    return (
        <div className="clisyc-wizard-success">
            ...
            <div className="clisyc-wizard-success-details">
                {primarySelection && (
                    <p><strong>{primarySelection.name}</strong></p>
                )}
                {dimensions.slice(1).map(dim => {
                    const sel = selections[dim.key];
                    return sel ? <p key={dim.key}>with <strong>{sel.name}</strong></p> : null;
                })}
                <p>
                    {date?.toLocaleDateString('en-US', { 
                        weekday: 'long', 
...
const SelectionSummary = ({ dimensions, selections, selectedDate, selectedTime }) => {
    const hasSelections = Object.keys(selections).length > 0 || selectedDate || selectedTime;
    ...
    return (
        <div className="clisyc-wizard-selection-summary">
            <div className="clisyc-wizard-selection-summary-title">Your Selections</div>
            <div className="clisyc-wizard-selection-summary-items">
                {dimensions.map(dim => {
                    const selection = selections[dim.key];
                    if (!selection) return null;
                    return (
                        <div key={dim.key} className="clisyc-wizard-selection-summary-item">
                            <span className="clisyc-wizard-selection-summary-label">{dim.singular_label}:</span>
                ...
                {selectedDate && (
                    <div className="clisyc-wizard-selection-summary-item">
                        <span className="clisyc-wizard-selection-summary-label">Date:</span>
                        <span className="clisyc-wizard-selection-summary-value">{formatDate(selectedDate)}</span>
                    </div>
                )}
                {selectedTime && (
                    <div className="clisyc-wizard-selection-summary-item">
                        <span className="clisyc-wizard-selection-summary-label">Time:</span>
                        <span className="clisyc-wizard-selection-summary-value">{selectedTime.time}</span>
                    </div>
                )}
            </div>
        </div>
...
const BookingWizard = ({ config }) => {
    ...
    const settingsUrl = config?.settingsUrl || '/wp-admin/admin.php?page=clisyc-dimensions';
        ...
        return (
            <div className="clisyc-booking-wizard">
                <div className="clisyc-wizard-no-dimensions">
                    ...
                    <h3 className="clisyc-wizard-no-dimensions-title">Setup Required</h3>
                    <p className="clisyc-wizard-no-dimensions-message">
                        The booking wizard requires at least one dimension (such as Service, Practitioner, or Room) to be configured and enabled.
                    </p>
                    <p className="clisyc-wizard-no-dimensions-instructions">
                        Please go to <strong>Client Sync → Dimensions</strong> in the WordPress admin to set up your booking dimensions.
                    </p>
                </div>
            </div>
        );
    ...
    useEffect(() => {
        ...
        if (currentStep === dateStepIndex && dimensions.length > 0) {
            const allSelected = dimensions.every(dim => dimensionSelections[dim.key]);
            if (allSelected) {
                fetchAvailableDates();
            }
        }
    ...
    const fetchAvailableDates = async () => {
        ...
        try {
            ...
            formData.append('start', startDate.toISOString().split('T')[0]);
    ...
    const fetchTimeSlots = async () => {
        ...
        try {
            ...
            formData.append('action', 'clisyc_get_calendar_events');
            formData.append('security', calendarNonce);
            formData.append('start', selectedDate.toISOString().split('T')[0]);
            formData.append('end', new Date(selectedDate.getTime() + 86400000).toISOString().split('T')[0]);
            formData.append('hide_booked', 'true');
    ...
    useEffect(() => {
        ...
        apiFetch({ path: `/clisyc/v1/seats/resolve/${selectedTime.slotId}` })
    ...
    const handleDimensionSelect = (dimensionKey, item) => {
        ...
        if (dimIndex >= 0 && dimIndex < dimensions.length - 1) {
            ...
            const keysToCheck = dimensions.slice(dimIndex + 1).map(d => d.key);
            const updatedSelections = { ...dimensionSelections, [dimensionKey]: item };
            ...
            keysToCheck.forEach(key => {
                ...
                if (selected) {
                    ...
                    const isCompatible = items.some(i => 
                        i.id === selected.id && 
                        (!i.compatibleWith || !i.compatibleWith[dimensionKey] || 
                         i.compatibleWith[dimensionKey].includes(item.id))
                    );
                    if (!isCompatible) {
    ...
    const canProceed = () => {
        ...
        switch (step.type) {
            case 'dimension':
                return dimensionSelections[step.dimension.key] !== undefined && 
            ...
            case 'details':
                return contactDetails.firstName && contactDetails.lastName && contactDetails.email;
            case 'confirm':
                return true;
            default:
                return false;
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            formData.append('time_slot_iso', selectedTime.start || timeSlotUtc);
            
            // Appointment date in Y-m-d format (from the UTC slot identifier)
            const appointmentDate = timeSlotUtc ? timeSlotUtc.split(' ')[0] : selectedDate.toISOString().split('T')[0];
            formData.append('appointment_date', appointmentDate);
            
            // Duration from selected service if available
            const primaryDim = dimensions[0];
            const primarySelection = primaryDim ? dimensionSelections[primaryDim.key] : null;
            const duration = primarySelection?.duration || selectedTime.durationMinutes || 60;
            ...
            if (!isLoggedIn) {
                formData.append('guest_name', `${contactDetails.firstName} ${contactDetails.lastName}`);
                formData.append('guest_email', contactDetails.email);
            }
            
            // Notes as custom field
            if (contactDetails.notes) {
                formData.append('clisyc_custom_field[notes]', contactDetails.notes);
            }

            // Seat selection data
            if (selectedSeats && selectedSeats.length > 0) {
                formData.append('selected_seats', JSON.stringify(selectedSeats));
                const seatToken = sessionStorage.getItem('clisyc_seat_session_token') || '';
            ...
            if (data.success) {
                ...
                setConfirmationNumber(data.data?.confirmation_number || data.data?.appointment_id);
                setIsComplete(true);
            } else {
                setSubmitError(data.data?.message || 'Failed to submit booking. Please try again.');
            }
        } catch (error) {
    ...
    const getPreviousSelections = (currentDimKey) => {
        ...
        for (const dim of dimensions) {
            if (dim.key === currentDimKey) break;
            ...
            case 'dimension':
                return (
                    <DimensionStep 
                        ...
                        selectedItem={dimensionSelections[step.dimension.key]}
                        onSelect={(item) => handleDimensionSelect(step.dimension.key, item)}
                        previousSelections={getPreviousSelections(step.dimension.key)}
                    />
                );
            case 'date':
                return (
    ...
    return (
        <div className="clisyc-booking-wizard">
            ...
            <div className="clisyc-wizard-content">
                {renderCurrentStep()}
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (container && window.clisycBookingWizardData) {
        ...
        root.render(<ErrorBoundary><BookingWizard config={window.clisycBookingWizardData} /></ErrorBoundary>);
    }
});
